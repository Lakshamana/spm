...
public class ProcessEventResourceIT {
  ...
  @Autowired private ProcessEventMapper processEventMapper;

  @Autowired private ProcessEventService processEventService;
  ...
  public void setup() {
    ...
    this.restProcessEventMockMvc =
        MockMvcBuilders.standaloneSetup(processEventResource)
  ...
  public void getAllProcessEvents() throws Exception {
    ...
    restProcessEventMockMvc
        .perform(get("/api/process-events?sort=id,desc"))
  ...
  public void getProcessEvent() throws Exception {
    ...
    restProcessEventMockMvc
        .perform(get("/api/process-events/{id}", processEvent.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingProcessEvent() throws Exception {
    // Get the processEvent
    restProcessEventMockMvc
        .perform(get("/api/process-events/{id}", Long.MAX_VALUE))
  ...
  public void updateProcessEvent() throws Exception {
    ...
    // Disconnect from session so that the updates on updatedProcessEvent are not directly saved in
    // db
  ...
  public void deleteProcessEvent() throws Exception {
    ...
    restProcessEventMockMvc
        .perform(
            delete("/api/process-events/{id}", processEvent.getId())
