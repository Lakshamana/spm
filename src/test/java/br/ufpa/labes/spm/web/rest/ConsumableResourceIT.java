...
public class ConsumableResourceIT {
  ...
  public void setup() {
    ...
    this.restConsumableMockMvc =
        MockMvcBuilders.standaloneSetup(consumableResource)
  ...
  public void getAllConsumables() throws Exception {
    ...
    restConsumableMockMvc
        .perform(get("/api/consumables?sort=id,desc"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        .andExpect(jsonPath("$.[*].id").value(hasItem(consumable.getId().intValue())))
        .andExpect(jsonPath("$.[*].unit").value(hasItem(DEFAULT_UNIT.toString())))
        .andExpect(
            jsonPath("$.[*].consumableStatus").value(hasItem(DEFAULT_CONSUMABLE_STATUS.toString())))
        .andExpect(
            jsonPath("$.[*].totalQuantity").value(hasItem(DEFAULT_TOTAL_QUANTITY.doubleValue())))
  ...
  public void getConsumable() throws Exception {
    ...
    // Get the consumable
    restConsumableMockMvc
        .perform(get("/api/consumables/{id}", consumable.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingConsumable() throws Exception {
    // Get the consumable
    restConsumableMockMvc
        .perform(get("/api/consumables/{id}", Long.MAX_VALUE))
  ...
  public void deleteConsumable() throws Exception {
    ...
    restConsumableMockMvc
        .perform(
            delete("/api/consumables/{id}", consumable.getId())
