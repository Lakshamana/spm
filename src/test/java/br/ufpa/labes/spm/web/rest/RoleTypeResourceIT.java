...
public class RoleTypeResourceIT {
  ...
  @Autowired private RoleTypeRepository roleTypeRepository;

  @Autowired private RoleTypeMapper roleTypeMapper;

  @Autowired private RoleTypeService roleTypeService;
  ...
  public void setup() {
    ...
    this.restRoleTypeMockMvc =
        MockMvcBuilders.standaloneSetup(roleTypeResource)
  ...
  public void getAllRoleTypes() throws Exception {
    ...
    restRoleTypeMockMvc
        .perform(get("/api/role-types?sort=id,desc"))
  ...
  public void getRoleType() throws Exception {
    ...
    // Get the roleType
    restRoleTypeMockMvc
        .perform(get("/api/role-types/{id}", roleType.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingRoleType() throws Exception {
    // Get the roleType
    restRoleTypeMockMvc
        .perform(get("/api/role-types/{id}", Long.MAX_VALUE))
  ...
  public void deleteRoleType() throws Exception {
    ...
    restRoleTypeMockMvc
        .perform(
            delete("/api/role-types/{id}", roleType.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
