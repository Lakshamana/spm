...
public class ArtifactTaskResourceIT {
  ...
  @Autowired private ArtifactTaskRepository artifactTaskRepository;

  @Autowired private ArtifactTaskMapper artifactTaskMapper;

  @Autowired private ArtifactTaskService artifactTaskService;
  ...
  public void setup() {
    ...
    this.restArtifactTaskMockMvc =
        MockMvcBuilders.standaloneSetup(artifactTaskResource)
  ...
  public void createArtifactTask() throws Exception {
    ...
    restArtifactTaskMockMvc
        .perform(
            post("/api/artifact-tasks")
  ...
  public void getAllArtifactTasks() throws Exception {
    ...
    restArtifactTaskMockMvc
        .perform(get("/api/artifact-tasks?sort=id,desc"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        .andExpect(jsonPath("$.[*].id").value(hasItem(artifactTask.getId().intValue())))
        .andExpect(
            jsonPath("$.[*].inWorkspaceVersion")
                .value(hasItem(DEFAULT_IN_WORKSPACE_VERSION.toString())))
        .andExpect(
            jsonPath("$.[*].outWorkspaceVersion")
                .value(hasItem(DEFAULT_OUT_WORKSPACE_VERSION.toString())));
  }
  ...
  public void getArtifactTask() throws Exception {
    ...
    restArtifactTaskMockMvc
        .perform(get("/api/artifact-tasks/{id}", artifactTask.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        ...
        .andExpect(
            jsonPath("$.outWorkspaceVersion").value(DEFAULT_OUT_WORKSPACE_VERSION.toString()));
  }
  ...
  public void getNonExistingArtifactTask() throws Exception {
    // Get the artifactTask
    restArtifactTaskMockMvc
        .perform(get("/api/artifact-tasks/{id}", Long.MAX_VALUE))
  ...
  public void updateArtifactTask() throws Exception {
    ...
    // Disconnect from session so that the updates on updatedArtifactTask are not directly saved in
    // db
  ...
  public void deleteArtifactTask() throws Exception {
    ...
    restArtifactTaskMockMvc
        .perform(
            delete("/api/artifact-tasks/{id}", artifactTask.getId())
