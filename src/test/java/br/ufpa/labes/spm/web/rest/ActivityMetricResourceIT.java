...
public class ActivityMetricResourceIT {
  ...
  @Autowired private ActivityMetricRepository activityMetricRepository;

  @Autowired private ActivityMetricMapper activityMetricMapper;

  @Autowired private ActivityMetricService activityMetricService;
  ...
  public void setup() {
    ...
    final ActivityMetricResource activityMetricResource =
        new ActivityMetricResource(activityMetricService);
    this.restActivityMetricMockMvc =
        MockMvcBuilders.standaloneSetup(activityMetricResource)
  ...
  public void createActivityMetric() throws Exception {
    ...
    restActivityMetricMockMvc
        .perform(
            post("/api/activity-metrics")
  ...
  public void getAllActivityMetrics() throws Exception {
    ...
    restActivityMetricMockMvc
        .perform(get("/api/activity-metrics?sort=id,desc"))
  ...
  public void getActivityMetric() throws Exception {
    ...
    restActivityMetricMockMvc
        .perform(get("/api/activity-metrics/{id}", activityMetric.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingActivityMetric() throws Exception {
    // Get the activityMetric
    restActivityMetricMockMvc
        .perform(get("/api/activity-metrics/{id}", Long.MAX_VALUE))
  ...
  public void updateActivityMetric() throws Exception {
    ...
    ActivityMetric updatedActivityMetric =
        activityMetricRepository.findById(activityMetric.getId()).get();
    // Disconnect from session so that the updates on updatedActivityMetric are not directly saved
    // in db
  ...
  public void deleteActivityMetric() throws Exception {
    ...
    restActivityMetricMockMvc
        .perform(
            delete("/api/activity-metrics/{id}", activityMetric.getId())
