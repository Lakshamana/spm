...
public class AuthorStatResourceIT {
  ...
  @Autowired private AuthorStatRepository authorStatRepository;

  @Autowired private AuthorStatMapper authorStatMapper;

  @Autowired private AuthorStatService authorStatService;
  ...
  public void setup() {
    ...
    this.restAuthorStatMockMvc =
        MockMvcBuilders.standaloneSetup(authorStatResource)
  ...
  public void getAllAuthorStats() throws Exception {
    ...
    restAuthorStatMockMvc
        .perform(get("/api/author-stats?sort=id,desc"))
  ...
  public void getAuthorStat() throws Exception {
    ...
    // Get the authorStat
    restAuthorStatMockMvc
        .perform(get("/api/author-stats/{id}", authorStat.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingAuthorStat() throws Exception {
    // Get the authorStat
    restAuthorStatMockMvc
        .perform(get("/api/author-stats/{id}", Long.MAX_VALUE))
  ...
  public void deleteAuthorStat() throws Exception {
    ...
    restAuthorStatMockMvc
        .perform(
            delete("/api/author-stats/{id}", authorStat.getId())
