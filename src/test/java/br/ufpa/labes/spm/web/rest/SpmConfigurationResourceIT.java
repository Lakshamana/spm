...
public class SpmConfigurationResourceIT {
  ...
  @Autowired private SpmConfigurationRepository spmConfigurationRepository;

  @Autowired private SpmConfigurationMapper spmConfigurationMapper;

  @Autowired private SpmConfigurationService spmConfigurationService;
  ...
  public void setup() {
    ...
    final SpmConfigurationResource spmConfigurationResource =
        new SpmConfigurationResource(spmConfigurationService);
    this.restSpmConfigurationMockMvc =
        MockMvcBuilders.standaloneSetup(spmConfigurationResource)
  ...
  public static SpmConfiguration createEntity(EntityManager em) {
    SpmConfiguration spmConfiguration =
        new SpmConfiguration()
  ...
  public void createSpmConfiguration() throws Exception {
    ...
    restSpmConfigurationMockMvc
        .perform(
            post("/api/spm-configurations")
    ...
    SpmConfiguration testSpmConfiguration =
        spmConfigurationList.get(spmConfigurationList.size() - 1);
    ...
    assertThat(testSpmConfiguration.isGraficoDeDesempenho())
        .isEqualTo(DEFAULT_GRAFICO_DE_DESEMPENHO);
  ...
  public void getAllSpmConfigurations() throws Exception {
    ...
    restSpmConfigurationMockMvc
        .perform(get("/api/spm-configurations?sort=id,desc"))
        ...
        .andExpect(
            jsonPath("$.[*].graficoDeEsforco")
                .value(hasItem(DEFAULT_GRAFICO_DE_ESFORCO.booleanValue())))
        .andExpect(
            jsonPath("$.[*].graficoDeCustos")
                .value(hasItem(DEFAULT_GRAFICO_DE_CUSTOS.booleanValue())))
        .andExpect(
            jsonPath("$.[*].graficoDeDesempenho")
                .value(hasItem(DEFAULT_GRAFICO_DE_DESEMPENHO.booleanValue())))
        .andExpect(
            jsonPath("$.[*].graficoDeTarefas")
                .value(hasItem(DEFAULT_GRAFICO_DE_TAREFAS.booleanValue())))
        .andExpect(
            jsonPath("$.[*].senhaEmRecuperacao")
                .value(hasItem(DEFAULT_SENHA_EM_RECUPERACAO.booleanValue())));
  }
  ...
  public void getSpmConfiguration() throws Exception {
    ...
    restSpmConfigurationMockMvc
        .perform(get("/api/spm-configurations/{id}", spmConfiguration.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        ...
        .andExpect(
            jsonPath("$.graficoDeDesempenho").value(DEFAULT_GRAFICO_DE_DESEMPENHO.booleanValue()))
        .andExpect(jsonPath("$.graficoDeTarefas").value(DEFAULT_GRAFICO_DE_TAREFAS.booleanValue()))
        .andExpect(
            jsonPath("$.senhaEmRecuperacao").value(DEFAULT_SENHA_EM_RECUPERACAO.booleanValue()));
  }
  ...
  public void getNonExistingSpmConfiguration() throws Exception {
    // Get the spmConfiguration
    restSpmConfigurationMockMvc
        .perform(get("/api/spm-configurations/{id}", Long.MAX_VALUE))
  ...
  public void updateSpmConfiguration() throws Exception {
    ...
    SpmConfiguration updatedSpmConfiguration =
        spmConfigurationRepository.findById(spmConfiguration.getId()).get();
    // Disconnect from session so that the updates on updatedSpmConfiguration are not directly saved
    // in db
    ...
    assertThat(testSpmConfiguration.isGraficoDeDesempenho())
        .isEqualTo(UPDATED_GRAFICO_DE_DESEMPENHO);
  ...
  public void deleteSpmConfiguration() throws Exception {
    ...
    restSpmConfigurationMockMvc
        .perform(
            delete("/api/spm-configurations/{id}", spmConfiguration.getId())
