...
public class NotWorkingDayResourceIT {
  ...
  @Autowired private NotWorkingDayRepository notWorkingDayRepository;
  ...
  public void setup() {
    ...
    final NotWorkingDayResource notWorkingDayResource =
        new NotWorkingDayResource(notWorkingDayRepository);
    this.restNotWorkingDayMockMvc =
        MockMvcBuilders.standaloneSetup(notWorkingDayResource)
  ...
  public static NotWorkingDay createEntity(EntityManager em) {
    NotWorkingDay notWorkingDay = new NotWorkingDay().name(DEFAULT_NAME);
  ...
  public static NotWorkingDay createUpdatedEntity(EntityManager em) {
    NotWorkingDay notWorkingDay = new NotWorkingDay().name(UPDATED_NAME);
    return notWorkingDay;
  }
  ...
  public void createNotWorkingDay() throws Exception {
    ...
    restNotWorkingDayMockMvc
        .perform(
            post("/api/not-working-days")
  ...
  public void getAllNotWorkingDays() throws Exception {
    ...
    restNotWorkingDayMockMvc
        .perform(get("/api/not-working-days?sort=id,desc"))
  ...
  public void getNotWorkingDay() throws Exception {
    ...
    restNotWorkingDayMockMvc
        .perform(get("/api/not-working-days/{id}", notWorkingDay.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingNotWorkingDay() throws Exception {
    // Get the notWorkingDay
    restNotWorkingDayMockMvc
        .perform(get("/api/not-working-days/{id}", Long.MAX_VALUE))
  ...
  public void updateNotWorkingDay() throws Exception {
    ...
    NotWorkingDay updatedNotWorkingDay =
        notWorkingDayRepository.findById(notWorkingDay.getId()).get();
    // Disconnect from session so that the updates on updatedNotWorkingDay are not directly saved in
    // db
    em.detach(updatedNotWorkingDay);
    updatedNotWorkingDay.name(UPDATED_NAME);

    restNotWorkingDayMockMvc
        .perform(
            put("/api/not-working-days")
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
  ...
  public void deleteNotWorkingDay() throws Exception {
    ...
    restNotWorkingDayMockMvc
        .perform(
            delete("/api/not-working-days/{id}", notWorkingDay.getId())
