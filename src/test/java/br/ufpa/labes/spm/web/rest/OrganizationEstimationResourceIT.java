...
public class OrganizationEstimationResourceIT {
  ...
  @Autowired private OrganizationEstimationRepository organizationEstimationRepository;

  @Autowired private OrganizationEstimationMapper organizationEstimationMapper;

  @Autowired private OrganizationEstimationService organizationEstimationService;
  ...
  public void setup() {
    ...
    final OrganizationEstimationResource organizationEstimationResource =
        new OrganizationEstimationResource(organizationEstimationService);
    this.restOrganizationEstimationMockMvc =
        MockMvcBuilders.standaloneSetup(organizationEstimationResource)
  ...
  public void createOrganizationEstimation() throws Exception {
    ...
    OrganizationEstimationDTO organizationEstimationDTO =
        organizationEstimationMapper.toDto(organizationEstimation);
    restOrganizationEstimationMockMvc
        .perform(
            post("/api/organization-estimations")
    ...
    List<OrganizationEstimation> organizationEstimationList =
        organizationEstimationRepository.findAll();
    assertThat(organizationEstimationList).hasSize(databaseSizeBeforeCreate + 1);
    OrganizationEstimation testOrganizationEstimation =
        organizationEstimationList.get(organizationEstimationList.size() - 1);
  }
  ...
  public void getAllOrganizationEstimations() throws Exception {
    ...
    restOrganizationEstimationMockMvc
        .perform(get("/api/organization-estimations?sort=id,desc"))
  ...
  public void getOrganizationEstimation() throws Exception {
    ...
    restOrganizationEstimationMockMvc
        .perform(get("/api/organization-estimations/{id}", organizationEstimation.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingOrganizationEstimation() throws Exception {
    // Get the organizationEstimation
    restOrganizationEstimationMockMvc
        .perform(get("/api/organization-estimations/{id}", Long.MAX_VALUE))
  ...
  public void updateOrganizationEstimation() throws Exception {
    ...
    OrganizationEstimation updatedOrganizationEstimation =
        organizationEstimationRepository.findById(organizationEstimation.getId()).get();
    // Disconnect from session so that the updates on updatedOrganizationEstimation are not directly
    // saved in db
    em.detach(updatedOrganizationEstimation);
    OrganizationEstimationDTO organizationEstimationDTO =
        organizationEstimationMapper.toDto(updatedOrganizationEstimation);

    restOrganizationEstimationMockMvc
        .perform(
            put("/api/organization-estimations")
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(organizationEstimationDTO)))
        .andExpect(status().isOk());
  ...
  public void deleteOrganizationEstimation() throws Exception {
    ...
    restOrganizationEstimationMockMvc
        .perform(
            delete("/api/organization-estimations/{id}", organizationEstimation.getId())
