...
public class RelationshipKindResourceIT {
  ...
  @Autowired private RelationshipKindRepository relationshipKindRepository;

  @Autowired private RelationshipKindMapper relationshipKindMapper;

  @Autowired private RelationshipKindService relationshipKindService;
  ...
  public void setup() {
    ...
    final RelationshipKindResource relationshipKindResource =
        new RelationshipKindResource(relationshipKindService);
    this.restRelationshipKindMockMvc =
        MockMvcBuilders.standaloneSetup(relationshipKindResource)
  ...
  public static RelationshipKind createEntity(EntityManager em) {
    RelationshipKind relationshipKind =
        new RelationshipKind().typeIdent(DEFAULT_TYPE_IDENT).description(DEFAULT_DESCRIPTION);
  ...
  public static RelationshipKind createUpdatedEntity(EntityManager em) {
    RelationshipKind relationshipKind =
        new RelationshipKind().typeIdent(UPDATED_TYPE_IDENT).description(UPDATED_DESCRIPTION);
    return relationshipKind;
  }
  ...
  public void createRelationshipKind() throws Exception {
    ...
    restRelationshipKindMockMvc
        .perform(
            post("/api/relationship-kinds")
    ...
    RelationshipKind testRelationshipKind =
        relationshipKindList.get(relationshipKindList.size() - 1);
  ...
  public void getAllRelationshipKinds() throws Exception {
    ...
    restRelationshipKindMockMvc
        .perform(get("/api/relationship-kinds?sort=id,desc"))
  ...
  public void getRelationshipKind() throws Exception {
    ...
    restRelationshipKindMockMvc
        .perform(get("/api/relationship-kinds/{id}", relationshipKind.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingRelationshipKind() throws Exception {
    // Get the relationshipKind
    restRelationshipKindMockMvc
        .perform(get("/api/relationship-kinds/{id}", Long.MAX_VALUE))
  ...
  public void updateRelationshipKind() throws Exception {
    ...
    RelationshipKind updatedRelationshipKind =
        relationshipKindRepository.findById(relationshipKind.getId()).get();
    // Disconnect from session so that the updates on updatedRelationshipKind are not directly saved
    // in db
    em.detach(updatedRelationshipKind);
    updatedRelationshipKind.typeIdent(UPDATED_TYPE_IDENT).description(UPDATED_DESCRIPTION);
    RelationshipKindDTO relationshipKindDTO = relationshipKindMapper.toDto(updatedRelationshipKind);

    restRelationshipKindMockMvc
        .perform(
            put("/api/relationship-kinds")
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(relationshipKindDTO)))
        .andExpect(status().isOk());
  ...
  public void deleteRelationshipKind() throws Exception {
    ...
    restRelationshipKindMockMvc
        .perform(
            delete("/api/relationship-kinds/{id}", relationshipKind.getId())
