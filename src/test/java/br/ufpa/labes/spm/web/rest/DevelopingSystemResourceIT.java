...
public class DevelopingSystemResourceIT {
  ...
  @Autowired private DevelopingSystemRepository developingSystemRepository;

  @Autowired private DevelopingSystemMapper developingSystemMapper;

  @Autowired private DevelopingSystemService developingSystemService;
  ...
  public void setup() {
    ...
    final DevelopingSystemResource developingSystemResource =
        new DevelopingSystemResource(developingSystemService);
    this.restDevelopingSystemMockMvc =
        MockMvcBuilders.standaloneSetup(developingSystemResource)
  ...
  public static DevelopingSystem createEntity(EntityManager em) {
    DevelopingSystem developingSystem =
        new DevelopingSystem()
            .ident(DEFAULT_IDENT)
            .name(DEFAULT_NAME)
  ...
  public void createDevelopingSystem() throws Exception {
    ...
    restDevelopingSystemMockMvc
        .perform(
            post("/api/developing-systems")
    ...
    DevelopingSystem testDevelopingSystem =
        developingSystemList.get(developingSystemList.size() - 1);
  ...
  public void getAllDevelopingSystems() throws Exception {
    ...
    restDevelopingSystemMockMvc
        .perform(get("/api/developing-systems?sort=id,desc"))
  ...
  public void getDevelopingSystem() throws Exception {
    ...
    restDevelopingSystemMockMvc
        .perform(get("/api/developing-systems/{id}", developingSystem.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingDevelopingSystem() throws Exception {
    // Get the developingSystem
    restDevelopingSystemMockMvc
        .perform(get("/api/developing-systems/{id}", Long.MAX_VALUE))
  ...
  public void updateDevelopingSystem() throws Exception {
    ...
    DevelopingSystem updatedDevelopingSystem =
        developingSystemRepository.findById(developingSystem.getId()).get();
    // Disconnect from session so that the updates on updatedDevelopingSystem are not directly saved
    // in db
  ...
  public void deleteDevelopingSystem() throws Exception {
    ...
    restDevelopingSystemMockMvc
        .perform(
            delete("/api/developing-systems/{id}", developingSystem.getId())
