...
public class TagStatsResourceIT {
  ...
  @Autowired private TagStatsRepository tagStatsRepository;
  ...
  public void setup() {
    ...
    this.restTagStatsMockMvc =
        MockMvcBuilders.standaloneSetup(tagStatsResource)
  ...
  public static TagStats createEntity(EntityManager em) {
    TagStats tagStats = new TagStats().count(DEFAULT_COUNT);
    return tagStats;
  }
  /**
  ...
  public static TagStats createUpdatedEntity(EntityManager em) {
    TagStats tagStats = new TagStats().count(UPDATED_COUNT);
    return tagStats;
  }
  ...
  public void getTagStats() throws Exception {
    ...
    // Get the tagStats
    restTagStatsMockMvc
        .perform(get("/api/tag-stats/{id}", tagStats.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void updateTagStats() throws Exception {
    ...
    updatedTagStats.count(UPDATED_COUNT);
  ...
  public void deleteTagStats() throws Exception {
    ...
    restTagStatsMockMvc
        .perform(
            delete("/api/tag-stats/{id}", tagStats.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
