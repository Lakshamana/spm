...
public class AgentWorkingLoadResourceIT {
  ...
  @Autowired private AgentWorkingLoadRepository agentWorkingLoadRepository;

  @Autowired private AgentWorkingLoadMapper agentWorkingLoadMapper;

  @Autowired private AgentWorkingLoadService agentWorkingLoadService;
  ...
  public void setup() {
    ...
    final AgentWorkingLoadResource agentWorkingLoadResource =
        new AgentWorkingLoadResource(agentWorkingLoadService);
    this.restAgentWorkingLoadMockMvc =
        MockMvcBuilders.standaloneSetup(agentWorkingLoadResource)
  ...
  public static AgentWorkingLoad createEntity(EntityManager em) {
    AgentWorkingLoad agentWorkingLoad =
        new AgentWorkingLoad().begin(DEFAULT_BEGIN).end(DEFAULT_END);
  ...
  public static AgentWorkingLoad createUpdatedEntity(EntityManager em) {
    AgentWorkingLoad agentWorkingLoad =
        new AgentWorkingLoad().begin(UPDATED_BEGIN).end(UPDATED_END);
    return agentWorkingLoad;
  }
  ...
  public void createAgentWorkingLoad() throws Exception {
    ...
    restAgentWorkingLoadMockMvc
        .perform(
            post("/api/agent-working-loads")
    ...
    AgentWorkingLoad testAgentWorkingLoad =
        agentWorkingLoadList.get(agentWorkingLoadList.size() - 1);
  ...
  public void getAllAgentWorkingLoads() throws Exception {
    ...
    restAgentWorkingLoadMockMvc
        .perform(get("/api/agent-working-loads?sort=id,desc"))
  ...
  public void getAgentWorkingLoad() throws Exception {
    ...
    restAgentWorkingLoadMockMvc
        .perform(get("/api/agent-working-loads/{id}", agentWorkingLoad.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingAgentWorkingLoad() throws Exception {
    // Get the agentWorkingLoad
    restAgentWorkingLoadMockMvc
        .perform(get("/api/agent-working-loads/{id}", Long.MAX_VALUE))
  ...
  public void updateAgentWorkingLoad() throws Exception {
    ...
    AgentWorkingLoad updatedAgentWorkingLoad =
        agentWorkingLoadRepository.findById(agentWorkingLoad.getId()).get();
    // Disconnect from session so that the updates on updatedAgentWorkingLoad are not directly saved
    // in db
    em.detach(updatedAgentWorkingLoad);
    updatedAgentWorkingLoad.begin(UPDATED_BEGIN).end(UPDATED_END);
    AgentWorkingLoadDTO agentWorkingLoadDTO = agentWorkingLoadMapper.toDto(updatedAgentWorkingLoad);

    restAgentWorkingLoadMockMvc
        .perform(
            put("/api/agent-working-loads")
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(agentWorkingLoadDTO)))
        .andExpect(status().isOk());
  ...
  public void deleteAgentWorkingLoad() throws Exception {
    ...
    restAgentWorkingLoadMockMvc
        .perform(
            delete("/api/agent-working-loads/{id}", agentWorkingLoad.getId())
