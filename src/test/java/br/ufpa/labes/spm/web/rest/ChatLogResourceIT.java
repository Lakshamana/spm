...
public class ChatLogResourceIT {
  ...
  @Mock private ChatLogRepository chatLogRepositoryMock;

  @Autowired private ChatLogMapper chatLogMapper;

  @Mock private ChatLogService chatLogServiceMock;
  ...
  public void setup() {
    ...
    this.restChatLogMockMvc =
        MockMvcBuilders.standaloneSetup(chatLogResource)
  ...
  public static ChatLog createEntity(EntityManager em) {
    ChatLog chatLog = new ChatLog().log(DEFAULT_LOG).date(DEFAULT_DATE);
    return chatLog;
  }
  /**
  ...
  public static ChatLog createUpdatedEntity(EntityManager em) {
    ChatLog chatLog = new ChatLog().log(UPDATED_LOG).date(UPDATED_DATE);
    return chatLog;
  }
  ...
  public void getAllChatLogs() throws Exception {
    ...
    restChatLogMockMvc
        .perform(get("/api/chat-logs?sort=id,desc"))
  ...
  public void getAllChatLogsWithEagerRelationshipsIsEnabled() throws Exception {
    ...
    when(chatLogServiceMock.findAllWithEagerRelationships(any()))
        .thenReturn(new PageImpl(new ArrayList<>()));

    MockMvc restChatLogMockMvc =
        MockMvcBuilders.standaloneSetup(chatLogResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
    ...
    restChatLogMockMvc.perform(get("/api/chat-logs?eagerload=true")).andExpect(status().isOk());
  ...
  public void getChatLog() throws Exception {
    ...
    // Get the chatLog
    restChatLogMockMvc
        .perform(get("/api/chat-logs/{id}", chatLog.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingChatLog() throws Exception {
    // Get the chatLog
    restChatLogMockMvc
        .perform(get("/api/chat-logs/{id}", Long.MAX_VALUE))
  ...
  public void updateChatLog() throws Exception {
    ...
    updatedChatLog.log(UPDATED_LOG).date(UPDATED_DATE);
  ...
  public void deleteChatLog() throws Exception {
    ...
    restChatLogMockMvc
        .perform(
            delete("/api/chat-logs/{id}", chatLog.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
