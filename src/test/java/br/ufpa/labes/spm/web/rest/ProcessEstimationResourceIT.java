...
public class ProcessEstimationResourceIT {
  ...
  @Autowired private ProcessEstimationRepository processEstimationRepository;

  @Autowired private ProcessEstimationMapper processEstimationMapper;

  @Autowired private ProcessEstimationService processEstimationService;
  ...
  public void setup() {
    ...
    final ProcessEstimationResource processEstimationResource =
        new ProcessEstimationResource(processEstimationService);
    this.restProcessEstimationMockMvc =
        MockMvcBuilders.standaloneSetup(processEstimationResource)
  ...
  public void createProcessEstimation() throws Exception {
    ...
    restProcessEstimationMockMvc
        .perform(
            post("/api/process-estimations")
    ...
    ProcessEstimation testProcessEstimation =
        processEstimationList.get(processEstimationList.size() - 1);
  }
  ...
  public void getAllProcessEstimations() throws Exception {
    ...
    restProcessEstimationMockMvc
        .perform(get("/api/process-estimations?sort=id,desc"))
  ...
  public void getProcessEstimation() throws Exception {
    ...
    restProcessEstimationMockMvc
        .perform(get("/api/process-estimations/{id}", processEstimation.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingProcessEstimation() throws Exception {
    // Get the processEstimation
    restProcessEstimationMockMvc
        .perform(get("/api/process-estimations/{id}", Long.MAX_VALUE))
  ...
  public void updateProcessEstimation() throws Exception {
    ...
    ProcessEstimation updatedProcessEstimation =
        processEstimationRepository.findById(processEstimation.getId()).get();
    // Disconnect from session so that the updates on updatedProcessEstimation are not directly
    // saved in db
    em.detach(updatedProcessEstimation);
    ProcessEstimationDTO processEstimationDTO =
        processEstimationMapper.toDto(updatedProcessEstimation);

    restProcessEstimationMockMvc
        .perform(
            put("/api/process-estimations")
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(processEstimationDTO)))
        .andExpect(status().isOk());
  ...
  public void deleteProcessEstimation() throws Exception {
    ...
    restProcessEstimationMockMvc
        .perform(
            delete("/api/process-estimations/{id}", processEstimation.getId())
