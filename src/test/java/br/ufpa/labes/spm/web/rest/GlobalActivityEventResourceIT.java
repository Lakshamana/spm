...
public class GlobalActivityEventResourceIT {
  ...
  @Autowired private GlobalActivityEventRepository globalActivityEventRepository;

  @Autowired private GlobalActivityEventMapper globalActivityEventMapper;

  @Autowired private GlobalActivityEventService globalActivityEventService;
  ...
  public void setup() {
    ...
    final GlobalActivityEventResource globalActivityEventResource =
        new GlobalActivityEventResource(globalActivityEventService);
    this.restGlobalActivityEventMockMvc =
        MockMvcBuilders.standaloneSetup(globalActivityEventResource)
  ...
  public void createGlobalActivityEvent() throws Exception {
    ...
    GlobalActivityEventDTO globalActivityEventDTO =
        globalActivityEventMapper.toDto(globalActivityEvent);
    restGlobalActivityEventMockMvc
        .perform(
            post("/api/global-activity-events")
    ...
    GlobalActivityEvent testGlobalActivityEvent =
        globalActivityEventList.get(globalActivityEventList.size() - 1);
  }
  ...
  public void getAllGlobalActivityEvents() throws Exception {
    ...
    restGlobalActivityEventMockMvc
        .perform(get("/api/global-activity-events?sort=id,desc"))
  ...
  public void getGlobalActivityEvent() throws Exception {
    ...
    restGlobalActivityEventMockMvc
        .perform(get("/api/global-activity-events/{id}", globalActivityEvent.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingGlobalActivityEvent() throws Exception {
    // Get the globalActivityEvent
    restGlobalActivityEventMockMvc
        .perform(get("/api/global-activity-events/{id}", Long.MAX_VALUE))
  ...
  public void updateGlobalActivityEvent() throws Exception {
    ...
    GlobalActivityEvent updatedGlobalActivityEvent =
        globalActivityEventRepository.findById(globalActivityEvent.getId()).get();
    // Disconnect from session so that the updates on updatedGlobalActivityEvent are not directly
    // saved in db
    em.detach(updatedGlobalActivityEvent);
    GlobalActivityEventDTO globalActivityEventDTO =
        globalActivityEventMapper.toDto(updatedGlobalActivityEvent);

    restGlobalActivityEventMockMvc
        .perform(
            put("/api/global-activity-events")
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(globalActivityEventDTO)))
        .andExpect(status().isOk());
  ...
  public void deleteGlobalActivityEvent() throws Exception {
    ...
    restGlobalActivityEventMockMvc
        .perform(
            delete("/api/global-activity-events/{id}", globalActivityEvent.getId())
