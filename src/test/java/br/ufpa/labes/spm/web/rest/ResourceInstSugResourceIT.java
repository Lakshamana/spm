...
public class ResourceInstSugResourceIT {
  ...
  @Mock private ResourceInstSugRepository resourceInstSugRepositoryMock;
  ...
  public void setup() {
    ...
    final ResourceInstSugResource resourceInstSugResource =
        new ResourceInstSugResource(resourceInstSugRepository);
  ...
  public void getAllResourceInstSugsWithEagerRelationshipsIsEnabled() throws Exception {
    ResourceInstSugResource resourceInstSugResource =
        new ResourceInstSugResource(resourceInstSugRepositoryMock);
    when(resourceInstSugRepositoryMock.findAllWithEagerRelationships(any()))
        .thenReturn(new PageImpl(new ArrayList<>()));

    MockMvc restResourceInstSugMockMvc =
        MockMvcBuilders.standaloneSetup(resourceInstSugResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
    ...
    restResourceInstSugMockMvc
        .perform(get("/api/resource-inst-sugs?eagerload=true"))
        .andExpect(status().isOk());
