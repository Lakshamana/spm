...
public class PluginResourceIT {
  ...
  @Mock private PluginRepository pluginRepositoryMock;

  @Autowired private PluginMapper pluginMapper;

  @Mock private PluginService pluginServiceMock;
  ...
  public void getAllPlugins() throws Exception {
    ...
    restPluginMockMvc
        ...
        .andExpect(
            jsonPath("$.[*].jsonConfigFile").value(hasItem(DEFAULT_JSON_CONFIG_FILE.toString())));
  }
  ...
  public void getAllPluginsWithEagerRelationshipsIsEnabled() throws Exception {
    ...
    when(pluginServiceMock.findAllWithEagerRelationships(any()))
        .thenReturn(new PageImpl(new ArrayList<>()));

    MockMvc restPluginMockMvc =
        MockMvcBuilders.standaloneSetup(pluginResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
    ...
    restPluginMockMvc.perform(get("/api/plugins?eagerload=true")).andExpect(status().isOk());
