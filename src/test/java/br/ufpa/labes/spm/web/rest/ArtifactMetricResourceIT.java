...
public class ArtifactMetricResourceIT {
  ...
  @Autowired private ArtifactMetricRepository artifactMetricRepository;

  @Autowired private ArtifactMetricMapper artifactMetricMapper;

  @Autowired private ArtifactMetricService artifactMetricService;
  ...
  public void setup() {
    ...
    final ArtifactMetricResource artifactMetricResource =
        new ArtifactMetricResource(artifactMetricService);
    this.restArtifactMetricMockMvc =
        MockMvcBuilders.standaloneSetup(artifactMetricResource)
  ...
  public void createArtifactMetric() throws Exception {
    ...
    restArtifactMetricMockMvc
        .perform(
            post("/api/artifact-metrics")
  ...
  public void getAllArtifactMetrics() throws Exception {
    ...
    restArtifactMetricMockMvc
        .perform(get("/api/artifact-metrics?sort=id,desc"))
  ...
  public void getArtifactMetric() throws Exception {
    ...
    restArtifactMetricMockMvc
        .perform(get("/api/artifact-metrics/{id}", artifactMetric.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingArtifactMetric() throws Exception {
    // Get the artifactMetric
    restArtifactMetricMockMvc
        .perform(get("/api/artifact-metrics/{id}", Long.MAX_VALUE))
  ...
  public void updateArtifactMetric() throws Exception {
    ...
    ArtifactMetric updatedArtifactMetric =
        artifactMetricRepository.findById(artifactMetric.getId()).get();
    // Disconnect from session so that the updates on updatedArtifactMetric are not directly saved
    // in db
  ...
  public void deleteArtifactMetric() throws Exception {
    ...
    restArtifactMetricMockMvc
        .perform(
            delete("/api/artifact-metrics/{id}", artifactMetric.getId())
