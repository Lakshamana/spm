...
public class WorkGroupMetricResourceIT {
  ...
  @Autowired private WorkGroupMetricRepository workGroupMetricRepository;

  @Autowired private WorkGroupMetricMapper workGroupMetricMapper;

  @Autowired private WorkGroupMetricService workGroupMetricService;
  ...
  public void setup() {
    ...
    final WorkGroupMetricResource workGroupMetricResource =
        new WorkGroupMetricResource(workGroupMetricService);
    this.restWorkGroupMetricMockMvc =
        MockMvcBuilders.standaloneSetup(workGroupMetricResource)
  ...
  public void createWorkGroupMetric() throws Exception {
    ...
    restWorkGroupMetricMockMvc
        .perform(
            post("/api/work-group-metrics")
  ...
  public void getAllWorkGroupMetrics() throws Exception {
    ...
    restWorkGroupMetricMockMvc
        .perform(get("/api/work-group-metrics?sort=id,desc"))
  ...
  public void getWorkGroupMetric() throws Exception {
    ...
    restWorkGroupMetricMockMvc
        .perform(get("/api/work-group-metrics/{id}", workGroupMetric.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingWorkGroupMetric() throws Exception {
    // Get the workGroupMetric
    restWorkGroupMetricMockMvc
        .perform(get("/api/work-group-metrics/{id}", Long.MAX_VALUE))
  ...
  public void updateWorkGroupMetric() throws Exception {
    ...
    WorkGroupMetric updatedWorkGroupMetric =
        workGroupMetricRepository.findById(workGroupMetric.getId()).get();
    // Disconnect from session so that the updates on updatedWorkGroupMetric are not directly saved
    // in db
  ...
  public void deleteWorkGroupMetric() throws Exception {
    ...
    restWorkGroupMetricMockMvc
        .perform(
            delete("/api/work-group-metrics/{id}", workGroupMetric.getId())
