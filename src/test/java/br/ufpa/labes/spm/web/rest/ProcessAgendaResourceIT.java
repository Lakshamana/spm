...
public class ProcessAgendaResourceIT {
  ...
  @Autowired private ProcessAgendaRepository processAgendaRepository;

  @Autowired private ProcessAgendaMapper processAgendaMapper;

  @Autowired private ProcessAgendaService processAgendaService;
  ...
  public void setup() {
    ...
    final ProcessAgendaResource processAgendaResource =
        new ProcessAgendaResource(processAgendaService);
    this.restProcessAgendaMockMvc =
        MockMvcBuilders.standaloneSetup(processAgendaResource)
  ...
  public void createProcessAgenda() throws Exception {
    ...
    restProcessAgendaMockMvc
        .perform(
            post("/api/process-agenda")
  ...
  public void getAllProcessAgenda() throws Exception {
    ...
    restProcessAgendaMockMvc
        .perform(get("/api/process-agenda?sort=id,desc"))
  ...
  public void getProcessAgenda() throws Exception {
    ...
    restProcessAgendaMockMvc
        .perform(get("/api/process-agenda/{id}", processAgenda.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingProcessAgenda() throws Exception {
    // Get the processAgenda
    restProcessAgendaMockMvc
        .perform(get("/api/process-agenda/{id}", Long.MAX_VALUE))
  ...
  public void updateProcessAgenda() throws Exception {
    ...
    ProcessAgenda updatedProcessAgenda =
        processAgendaRepository.findById(processAgenda.getId()).get();
    // Disconnect from session so that the updates on updatedProcessAgenda are not directly saved in
    // db
  ...
  public void deleteProcessAgenda() throws Exception {
    ...
    restProcessAgendaMockMvc
        .perform(
            delete("/api/process-agenda/{id}", processAgenda.getId())
