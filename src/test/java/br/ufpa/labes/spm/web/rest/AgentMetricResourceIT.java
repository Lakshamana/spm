...
public class AgentMetricResourceIT {
  ...
  @Autowired private AgentMetricRepository agentMetricRepository;

  @Autowired private AgentMetricMapper agentMetricMapper;

  @Autowired private AgentMetricService agentMetricService;
  ...
  public void setup() {
    ...
    this.restAgentMetricMockMvc =
        MockMvcBuilders.standaloneSetup(agentMetricResource)
  ...
  public void getAllAgentMetrics() throws Exception {
    ...
    restAgentMetricMockMvc
        .perform(get("/api/agent-metrics?sort=id,desc"))
  ...
  public void getAgentMetric() throws Exception {
    ...
    // Get the agentMetric
    restAgentMetricMockMvc
        .perform(get("/api/agent-metrics/{id}", agentMetric.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingAgentMetric() throws Exception {
    // Get the agentMetric
    restAgentMetricMockMvc
        .perform(get("/api/agent-metrics/{id}", Long.MAX_VALUE))
  ...
  public void updateAgentMetric() throws Exception {
    ...
    // Disconnect from session so that the updates on updatedAgentMetric are not directly saved in
    // db
  ...
  public void deleteAgentMetric() throws Exception {
    ...
    restAgentMetricMockMvc
        .perform(
            delete("/api/agent-metrics/{id}", agentMetric.getId())
