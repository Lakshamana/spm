...
public class MetricDefinitionUnitResourceIT {
  ...
  @Autowired private MetricDefinitionUnitRepository metricDefinitionUnitRepository;

  @Autowired private MetricDefinitionUnitMapper metricDefinitionUnitMapper;

  @Autowired private MetricDefinitionUnitService metricDefinitionUnitService;
  ...
  public void setup() {
    ...
    final MetricDefinitionUnitResource metricDefinitionUnitResource =
        new MetricDefinitionUnitResource(metricDefinitionUnitService);
    this.restMetricDefinitionUnitMockMvc =
        MockMvcBuilders.standaloneSetup(metricDefinitionUnitResource)
  ...
  public static MetricDefinitionUnit createEntity(EntityManager em) {
    MetricDefinitionUnit metricDefinitionUnit = new MetricDefinitionUnit().unit(DEFAULT_UNIT);
  ...
  public static MetricDefinitionUnit createUpdatedEntity(EntityManager em) {
    MetricDefinitionUnit metricDefinitionUnit = new MetricDefinitionUnit().unit(UPDATED_UNIT);
    return metricDefinitionUnit;
  }
  ...
  public void createMetricDefinitionUnit() throws Exception {
    ...
    MetricDefinitionUnitDTO metricDefinitionUnitDTO =
        metricDefinitionUnitMapper.toDto(metricDefinitionUnit);
    restMetricDefinitionUnitMockMvc
        .perform(
            post("/api/metric-definition-units")
    ...
    MetricDefinitionUnit testMetricDefinitionUnit =
        metricDefinitionUnitList.get(metricDefinitionUnitList.size() - 1);
  ...
  public void getAllMetricDefinitionUnits() throws Exception {
    ...
    restMetricDefinitionUnitMockMvc
        .perform(get("/api/metric-definition-units?sort=id,desc"))
  ...
  public void getMetricDefinitionUnit() throws Exception {
    ...
    restMetricDefinitionUnitMockMvc
        .perform(get("/api/metric-definition-units/{id}", metricDefinitionUnit.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingMetricDefinitionUnit() throws Exception {
    // Get the metricDefinitionUnit
    restMetricDefinitionUnitMockMvc
        .perform(get("/api/metric-definition-units/{id}", Long.MAX_VALUE))
  ...
  public void updateMetricDefinitionUnit() throws Exception {
    ...
    MetricDefinitionUnit updatedMetricDefinitionUnit =
        metricDefinitionUnitRepository.findById(metricDefinitionUnit.getId()).get();
    // Disconnect from session so that the updates on updatedMetricDefinitionUnit are not directly
    // saved in db
    em.detach(updatedMetricDefinitionUnit);
    updatedMetricDefinitionUnit.unit(UPDATED_UNIT);
    MetricDefinitionUnitDTO metricDefinitionUnitDTO =
        metricDefinitionUnitMapper.toDto(updatedMetricDefinitionUnit);

    restMetricDefinitionUnitMockMvc
        .perform(
            put("/api/metric-definition-units")
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
                .content(TestUtil.convertObjectToJsonBytes(metricDefinitionUnitDTO)))
        .andExpect(status().isOk());
  ...
  public void deleteMetricDefinitionUnit() throws Exception {
    ...
    restMetricDefinitionUnitMockMvc
        .perform(
            delete("/api/metric-definition-units/{id}", metricDefinitionUnit.getId())
