...
public class ConnectionEventResourceIT {
  ...
  @Autowired private ConnectionEventRepository connectionEventRepository;

  @Autowired private ConnectionEventMapper connectionEventMapper;

  @Autowired private ConnectionEventService connectionEventService;
  ...
  public void setup() {
    ...
    final ConnectionEventResource connectionEventResource =
        new ConnectionEventResource(connectionEventService);
    this.restConnectionEventMockMvc =
        MockMvcBuilders.standaloneSetup(connectionEventResource)
  ...
  public void createConnectionEvent() throws Exception {
    ...
    restConnectionEventMockMvc
        .perform(
            post("/api/connection-events")
  ...
  public void getAllConnectionEvents() throws Exception {
    ...
    restConnectionEventMockMvc
        .perform(get("/api/connection-events?sort=id,desc"))
  ...
  public void getConnectionEvent() throws Exception {
    ...
    restConnectionEventMockMvc
        .perform(get("/api/connection-events/{id}", connectionEvent.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingConnectionEvent() throws Exception {
    // Get the connectionEvent
    restConnectionEventMockMvc
        .perform(get("/api/connection-events/{id}", Long.MAX_VALUE))
  ...
  public void updateConnectionEvent() throws Exception {
    ...
    ConnectionEvent updatedConnectionEvent =
        connectionEventRepository.findById(connectionEvent.getId()).get();
    // Disconnect from session so that the updates on updatedConnectionEvent are not directly saved
    // in db
  ...
  public void deleteConnectionEvent() throws Exception {
    ...
    restConnectionEventMockMvc
        .perform(
            delete("/api/connection-events/{id}", connectionEvent.getId())
