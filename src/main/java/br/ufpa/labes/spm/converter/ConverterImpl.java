...
/**
 * Classe respons�vel por implementar os servi�os disponibilizados na Interface Converter.
 * Nesta classe est�o implementados os servi�os de convers�o de Entidades em DTO e DTO em
 * Entidades. A finalidade desses servi�os � facilitar o trabalho do programador na convers�o
 * dos referidos tipos, uma vez que o progamador n�o precisa fazer todo o trabalho manualmente.
 *
 * @author Breno Fran�a
...
public class ConverterImpl implements Converter {
	 ...
	 *
	 * M�todo getDTO. Este m�todo recebe uma lista de Entidades, que possuem informa��es
	 * necess�rias para a execu��o de alguma a��o dentro do sistema, e retorna um DTO
	 * que cont�m todas as informa��es relevantes provenientes das Entidades passadas
	 * como par�metro. O mapeamento de quais informa��es de uma entidade dever�o estar
	 * presentes no DTO � feito atrav�s de Annotation que s�o inseridas no DTO e a partir
	 * das quais � poss�vel fazer a convers�o tanto de Entidades em DTO, quanto de um
	 * DTO para Entidades.
	 *
	 ...
	 * @throws - Exce��o no caso de um determinado atributo da classe do DTO a ser instanciado n�o estiver mapeado
	 * @throws - Exce��o no caso de um atributo mapeado no DTO n�o possuir m�todo get p�blico em sua Entidade de origem
	...
	public Object getDTO(List<?> entities, Class<?> dtoClass) throws ImplementationException {
		...
		try {
			...
			throw new ImplementationException("O objeto DTO da classe '" + dtoClass + "' nao pode ser instanciado!");
		}
		catch (IllegalAccessException e) {
			throw new ImplementationException("O objeto DTO da classe '" + dtoClass
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (mapping == null)
				{
				throw new ImplementationException("O atributo '"+field.getName()+"' da classe '" + dtoClass + "' nao possui mapeamento. \n Adicione @IgnoreMapping ou o mapeamento correspondente.");
				}

			// Entity class
			Class<?> eClass = mapping.entityClass();
			// obtem objeto que contem atributo a ser adicionado ao DTO
			Object entity = this.getElementOfClass(entities, eClass);//TODO: COLOCAR TESTE PRA ANNOTATION, PARA VERIFICAR SE MESMO SENDO NULO VAI IGNORAR OU VAI SER OBRIGATORIO
			if(entity==null ){
				if(isRequired)
				{
					//excecao pq um campo required nao tem classe que possa preenche-lo
					throw new ImplementationException("O atributo '"+field.getName()+"' da classe '" + dtoClass + "' � obrigatorio. \n Uma inst�ncia da classe '"+eClass+ "' precisa ser par�metro desta convers�o.");
				}else {
			...
			if (entityGetter == null) {
				...
				throw new ImplementationException("Nao existe um metodo 'get' para o atributo '" + attribute + "' na classe '"
			...
			try {
				...
				throw new ImplementationException("Nao foi possivel chamar m�todo 'get'. Este metodo nao deve possuir parametros.");
			}
			catch (IllegalAccessException e) {
				...
				throw new ImplementationException("Nao foi possivel chamar atributo que nao existe ou nao possui um metodo 'get'.");
			}
		}
	...
	public Object getDTO(Object entity, Class<?> dtoClass) throws ImplementationException {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (entityGetter == null) {
				// erro, pois o mapeamento aponta para um atributo que nao
				// possui get ou nao existe
				throw new ImplementationException("Nao existe um metodo 'get' para o atributo '" + field.getName() + "' na classe '"
	...
	/**
	 *
	 * M�todo getEntities. Este m�todo recebe um DTO, que possui informa��es de v�rias
	 * Entidades do modelo necess�rias para a execu��o de alguma a��o dentro do sistema,
	 * e retorna uma lista com essas diversas Entidades com suas devidas informa��es
	 * prevenientes do DTO. O mapeamento de quais informa��es no TDO dever�o estar presentes
	 * em quais Entidades � feito atrav�s de Annotation que s�o inseridas no DTO e a partir
	 * das quais � poss�vel fazer a convers�o tanto de Entidades em DTO, quanto de um
	 * DTO para Entidades.
	 *
	...
	public List<?> getEntities(Object dto) throws ImplementationException {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (mapping == null)
				{
				throw new ImplementationException("O atributo '"+field.getName()+"' da classe '" + dto.getClass() + "' nao possui mapeamento. \n Adicione @IgnoreMapping ou o mapeamento correspondente.");
				}
			...
			if (entity == null) {
				try {
					...
					throw new ImplementationException("O objeto da classe '" + eClass + "' nao pode ser instanciado!");
				}
				catch (IllegalAccessException e) {
					throw new ImplementationException("O objeto da classe '" + eClass
							+ "' nao pode ser instanciado! Verifique se existe construtor publico vazio");
				}
			...
			try {
				...
				throw new ImplementationException("O mapeamento aponta para um atributo que nao existe ou nao possui um metodo 'set'.");
			}
			catch (IllegalAccessException e) {
				throw new ImplementationException("O mapeamento aponta para um atributo que nao existe ou nao possui um metodo 'set'.");
			}
	...
	public Object getEntity(Object dto, Class destClass) throws ImplementationException {
		...
		try {
			if(destClass!=null){
				...
				throw new ImplementationException("� necess�rio o nome de uma classe v�lida para invocar este m�todo! ");
			}
	 ...
	 * @param attribute - Nome do atributo o qual o m�todo 'get' busca na classe. O nome do
	 * atributo ir� compor o nome do m�todo 'get' ficando este da seguinte forma: getNomeDoAtributo.
	 *
	...
	private Method getter(Class<?> clazz, String attribute) throws ImplementationException {
		...
		try {
			...
			throw new ImplementationException("Problema de seguranca ao acessar o m�todo '" + methodName + "' na classe '"
					+ clazz.getSimpleName() + "'.");
		}
		catch (NoSuchMethodException e) {
			throw new ImplementationException("O m�todo '" + methodName + "' n�o foi encontrado na classe '" + clazz.getSimpleName() + "'.");
		}
		return getter;
	}
	 ...
	 * atributo ir� compor o nome do m�todo 'set' ficando este da seguinte forma: setNomeDoAtributo.
	 *
	...
	private Method setter(Class<?> clazz, String attribute) throws ImplementationException {
		...
		try {
			...
			throw new ImplementationException("Problema de seguranca ao acessar o m�todo '" + methodName + "' na classe '"
					+ clazz.getSimpleName() + "' com parametro do tipo '" + field.getType().getName() + "'.");
		}
		catch (NoSuchMethodException e) {
			throw new ImplementationException("O m�todo '" + methodName + "' n�o foi encontrado na classe '" + clazz.getSimpleName()
					+ "'.");
	...
	private Field getField(Class<?> clazz, String name) throws ImplementationException {
		try {
			...
			throw new ImplementationException("Problema de seguranca ao acessar o atributo '" + name + "' na classe '"
					+ clazz.getSimpleName() + "'.");
