...
public class ResourceTypeResource {
    ...
    public List<ResourceTypeDTO> getAllResourceTypes(@RequestParam(required = false) String filter) {
        if ("thetypesuper-is-null".equals(filter)) {
            log.debug("REST request to get all ResourceTypes where theTypeSuper is null");
            return resourceTypeService.findAllWhereTheTypeSuperIsNull();
        }
