...
public class CompanyUnitResource {
   ...
   *     companyUnit, or with status {@code 400 (Bad Request)} if the companyUnit has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/company-units")
  public ResponseEntity<CompanyUnit> createCompanyUnit(@RequestBody CompanyUnit companyUnit)
   ...
   *     companyUnit, or with status {@code 400 (Bad Request)} if the companyUnit is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the companyUnit couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/company-units")
  public ResponseEntity<CompanyUnit> updateCompanyUnit(@RequestBody CompanyUnit companyUnit)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, companyUnit.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the companyUnit,
