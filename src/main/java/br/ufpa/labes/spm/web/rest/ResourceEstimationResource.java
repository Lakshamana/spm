...
public class ResourceEstimationResource {
   ...
   *     resourceEstimationDTO, or with status {@code 400 (Bad Request)} if the resourceEstimation
   *     has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/resource-estimations")
  public ResponseEntity<ResourceEstimationDTO> createResourceEstimation(
      @RequestBody ResourceEstimationDTO resourceEstimationDTO) throws URISyntaxException {
    log.debug("REST request to save ResourceEstimation : {}", resourceEstimationDTO);
    if (resourceEstimationDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new resourceEstimation cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     resourceEstimationDTO, or with status {@code 400 (Bad Request)} if the
   *     resourceEstimationDTO is not valid, or with status {@code 500 (Internal Server Error)} if
   *     the resourceEstimationDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/resource-estimations")
  public ResponseEntity<ResourceEstimationDTO> updateResourceEstimation(
      @RequestBody ResourceEstimationDTO resourceEstimationDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, resourceEstimationDTO.getId().toString()))
        .body(result);
  }
  ...
  public List<ResourceEstimationDTO> getAllResourceEstimations(
    ...
    if ("theestimationsuper-is-null".equals(filter)) {
      log.debug("REST request to get all ResourceEstimations where theEstimationSuper is null");
      return resourceEstimationService.findAllWhereTheEstimationSuperIsNull();
    }
   ...
   *     resourceEstimationDTO, or with status {@code 404 (Not Found)}.
   */
