...
public class CatalogEventResource {
   ...
   *     catalogEventDTO, or with status {@code 400 (Bad Request)} if the catalogEvent has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/catalog-events")
  public ResponseEntity<CatalogEventDTO> createCatalogEvent(
      @RequestBody CatalogEventDTO catalogEventDTO) throws URISyntaxException {
    log.debug("REST request to save CatalogEvent : {}", catalogEventDTO);
    if (catalogEventDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new catalogEvent cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     catalogEventDTO, or with status {@code 400 (Bad Request)} if the catalogEventDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the catalogEventDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/catalog-events")
  public ResponseEntity<CatalogEventDTO> updateCatalogEvent(
      @RequestBody CatalogEventDTO catalogEventDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, catalogEventDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of catalogEvents
   *     in body.
   */
  @GetMapping("/catalog-events")
  public List<CatalogEventDTO> getAllCatalogEvents(@RequestParam(required = false) String filter) {
    if ("theeventsuper-is-null".equals(filter)) {
      log.debug("REST request to get all CatalogEvents where theEventSuper is null");
      return catalogEventService.findAllWhereTheEventSuperIsNull();
    }
   ...
   *     catalogEventDTO, or with status {@code 404 (Not Found)}.
   */
