...
public class EventTypeResource {
   ...
   *     eventTypeDTO, or with status {@code 400 (Bad Request)} if the eventType has already an ID.
  ...
  public ResponseEntity<EventTypeDTO> createEventType(@RequestBody EventTypeDTO eventTypeDTO)
    ...
    if (eventTypeDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new eventType cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     eventTypeDTO, or with status {@code 400 (Bad Request)} if the eventTypeDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the eventTypeDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, eventTypeDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of eventTypes in
   *     body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the eventTypeDTO,
