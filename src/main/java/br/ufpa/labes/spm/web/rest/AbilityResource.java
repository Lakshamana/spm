...
public class AbilityResource {
   ...
   *     abilityDTO, or with status {@code 400 (Bad Request)} if the ability has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/abilities")
  public ResponseEntity<AbilityDTO> createAbility(@RequestBody AbilityDTO abilityDTO)
      throws URISyntaxException {
    log.debug("REST request to save Ability : {}", abilityDTO);
    if (abilityDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new ability cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     abilityDTO, or with status {@code 400 (Bad Request)} if the abilityDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the abilityDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, abilityDTO.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of abilities in
   *     body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the abilityDTO,
