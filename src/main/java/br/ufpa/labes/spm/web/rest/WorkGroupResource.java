...
public class WorkGroupResource {
   ...
   *     workGroupDTO, or with status {@code 400 (Bad Request)} if the workGroup has already an ID.
   ...
   *     workGroupDTO, or with status {@code 400 (Bad Request)} if the workGroupDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the workGroupDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/work-groups")
  public ResponseEntity<WorkGroupDTO> updateWorkGroup(@RequestBody WorkGroupDTO workGroupDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, workGroupDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the workGroupDTO,
