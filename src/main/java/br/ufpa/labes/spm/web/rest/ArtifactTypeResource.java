...
public class ArtifactTypeResource {
  ...
  public List<ArtifactTypeDTO> getAllArtifactTypes(@RequestParam(required = false) String filter) {
    if ("thetypesuper-is-null".equals(filter)) {
      log.debug("REST request to get all ArtifactTypes where theTypeSuper is null");
      return artifactTypeService.findAllWhereTheTypeSuperIsNull();
    }
