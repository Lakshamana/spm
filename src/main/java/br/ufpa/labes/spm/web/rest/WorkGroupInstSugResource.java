...
public class WorkGroupInstSugResource {
   ...
   *     workGroupInstSug, or with status {@code 400 (Bad Request)} if the workGroupInstSug has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/work-group-inst-sugs")
  public ResponseEntity<WorkGroupInstSug> createWorkGroupInstSug(
      @RequestBody WorkGroupInstSug workGroupInstSug) throws URISyntaxException {
    log.debug("REST request to save WorkGroupInstSug : {}", workGroupInstSug);
    if (workGroupInstSug.getId() != null) {
      throw new BadRequestAlertException(
          "A new workGroupInstSug cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     workGroupInstSug, or with status {@code 400 (Bad Request)} if the workGroupInstSug is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the workGroupInstSug couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/work-group-inst-sugs")
  public ResponseEntity<WorkGroupInstSug> updateWorkGroupInstSug(
      @RequestBody WorkGroupInstSug workGroupInstSug) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, workGroupInstSug.getId().toString()))
        .body(result);
  }
   ...
   *     workGroupInstSugs in body.
   */
  @GetMapping("/work-group-inst-sugs")
  public List<WorkGroupInstSug> getAllWorkGroupInstSugs(
   ...
   *     workGroupInstSug, or with status {@code 404 (Not Found)}.
   */
  ...
  public ResponseEntity<WorkGroupInstSug> getWorkGroupInstSug(@PathVariable Long id) {
    ...
    Optional<WorkGroupInstSug> workGroupInstSug =
        workGroupInstSugRepository.findOneWithEagerRelationships(id);
