...
public class ConnectionEventResource {
   ...
   *     connectionEventDTO, or with status {@code 400 (Bad Request)} if the connectionEvent has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/connection-events")
  public ResponseEntity<ConnectionEventDTO> createConnectionEvent(
      @RequestBody ConnectionEventDTO connectionEventDTO) throws URISyntaxException {
    log.debug("REST request to save ConnectionEvent : {}", connectionEventDTO);
    if (connectionEventDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new connectionEvent cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     connectionEventDTO, or with status {@code 400 (Bad Request)} if the connectionEventDTO is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the connectionEventDTO
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/connection-events")
  public ResponseEntity<ConnectionEventDTO> updateConnectionEvent(
      @RequestBody ConnectionEventDTO connectionEventDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, connectionEventDTO.getId().toString()))
        .body(result);
  }
   ...
   *     connectionEventDTO, or with status {@code 404 (Not Found)}.
   */
