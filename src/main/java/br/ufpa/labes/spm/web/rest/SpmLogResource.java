...
public class SpmLogResource {
   ...
   *     spmLogDTO, or with status {@code 400 (Bad Request)} if the spmLog has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/spm-logs")
  public ResponseEntity<SpmLogDTO> createSpmLog(@RequestBody SpmLogDTO spmLogDTO)
      throws URISyntaxException {
    log.debug("REST request to save SpmLog : {}", spmLogDTO);
    if (spmLogDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new spmLog cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     spmLogDTO, or with status {@code 400 (Bad Request)} if the spmLogDTO is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the spmLogDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/spm-logs")
  public ResponseEntity<SpmLogDTO> updateSpmLog(@RequestBody SpmLogDTO spmLogDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, spmLogDTO.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of spmLogs in
   *     body.
   */
  @GetMapping("/spm-logs")
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the spmLogDTO, or
