...
public class OutOfWorkPeriodResource {
   ...
   *     outOfWorkPeriodDTO, or with status {@code 400 (Bad Request)} if the outOfWorkPeriod has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/out-of-work-periods")
  public ResponseEntity<OutOfWorkPeriodDTO> createOutOfWorkPeriod(
      @RequestBody OutOfWorkPeriodDTO outOfWorkPeriodDTO) throws URISyntaxException {
    log.debug("REST request to save OutOfWorkPeriod : {}", outOfWorkPeriodDTO);
    if (outOfWorkPeriodDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new outOfWorkPeriod cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     outOfWorkPeriodDTO, or with status {@code 400 (Bad Request)} if the outOfWorkPeriodDTO is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the outOfWorkPeriodDTO
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/out-of-work-periods")
  public ResponseEntity<OutOfWorkPeriodDTO> updateOutOfWorkPeriod(
      @RequestBody OutOfWorkPeriodDTO outOfWorkPeriodDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, outOfWorkPeriodDTO.getId().toString()))
        .body(result);
  }
   ...
   *     outOfWorkPeriodDTO, or with status {@code 404 (Not Found)}.
   */
