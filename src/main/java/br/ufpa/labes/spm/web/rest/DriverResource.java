...
public class DriverResource {
   ...
   *     driverDTO, or with status {@code 400 (Bad Request)} if the driver has already an ID.
   ...
   *     driverDTO, or with status {@code 400 (Bad Request)} if the driverDTO is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the driverDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/drivers")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, driverDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the driverDTO, or
