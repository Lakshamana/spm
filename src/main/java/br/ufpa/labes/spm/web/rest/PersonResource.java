...
public class PersonResource {
   ...
   *     person, or with status {@code 400 (Bad Request)} if the person has already an ID.
   ...
   *     person, or with status {@code 400 (Bad Request)} if the person is not valid, or with status
   *     {@code 500 (Internal Server Error)} if the person couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/people")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, person.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the person, or
