...
public class ResourceMetricResource {
   ...
   *     resourceMetricDTO, or with status {@code 400 (Bad Request)} if the resourceMetric has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/resource-metrics")
  public ResponseEntity<ResourceMetricDTO> createResourceMetric(
      @RequestBody ResourceMetricDTO resourceMetricDTO) throws URISyntaxException {
    log.debug("REST request to save ResourceMetric : {}", resourceMetricDTO);
    if (resourceMetricDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new resourceMetric cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     resourceMetricDTO, or with status {@code 400 (Bad Request)} if the resourceMetricDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the resourceMetricDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/resource-metrics")
  public ResponseEntity<ResourceMetricDTO> updateResourceMetric(
      @RequestBody ResourceMetricDTO resourceMetricDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, resourceMetricDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of resourceMetrics
   *     in body.
   */
  ...
  public List<ResourceMetricDTO> getAllResourceMetrics(
    ...
    if ("themetricsuper-is-null".equals(filter)) {
      log.debug("REST request to get all ResourceMetrics where theMetricSuper is null");
      return resourceMetricService.findAllWhereTheMetricSuperIsNull();
    }
   ...
   *     resourceMetricDTO, or with status {@code 404 (Not Found)}.
   */
