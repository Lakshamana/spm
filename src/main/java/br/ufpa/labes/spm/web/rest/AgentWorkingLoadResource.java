...
public class AgentWorkingLoadResource {
   ...
   *     agentWorkingLoadDTO, or with status {@code 400 (Bad Request)} if the agentWorkingLoad has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/agent-working-loads")
  public ResponseEntity<AgentWorkingLoadDTO> createAgentWorkingLoad(
      @RequestBody AgentWorkingLoadDTO agentWorkingLoadDTO) throws URISyntaxException {
    log.debug("REST request to save AgentWorkingLoad : {}", agentWorkingLoadDTO);
    if (agentWorkingLoadDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new agentWorkingLoad cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     agentWorkingLoadDTO, or with status {@code 400 (Bad Request)} if the agentWorkingLoadDTO is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the agentWorkingLoadDTO
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/agent-working-loads")
  public ResponseEntity<AgentWorkingLoadDTO> updateAgentWorkingLoad(
      @RequestBody AgentWorkingLoadDTO agentWorkingLoadDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, agentWorkingLoadDTO.getId().toString()))
        .body(result);
  }
   ...
   *     agentWorkingLoadDTO, or with status {@code 404 (Not Found)}.
   */
