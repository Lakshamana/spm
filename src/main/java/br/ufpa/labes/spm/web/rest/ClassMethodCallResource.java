...
public class ClassMethodCallResource {
    ...
    public List<ClassMethodCallDTO> getAllClassMethodCalls(@RequestParam(required = false) String filter) {
        if ("thesubroutinesuper-is-null".equals(filter)) {
            log.debug("REST request to get all ClassMethodCalls where theSubroutineSuper is null");
            return classMethodCallService.findAllWhereTheSubroutineSuperIsNull();
        }
