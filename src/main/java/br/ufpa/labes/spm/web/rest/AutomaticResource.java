...
public class AutomaticResource {
   ...
   *     automatic, or with status {@code 400 (Bad Request)} if the automatic has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/automatics")
  public ResponseEntity<Automatic> createAutomatic(@RequestBody Automatic automatic)
   ...
   *     automatic, or with status {@code 400 (Bad Request)} if the automatic is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the automatic couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/automatics")
  public ResponseEntity<Automatic> updateAutomatic(@RequestBody Automatic automatic)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, automatic.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the automatic, or
