...
public class ResourcePossibleUseResource {
   ...
   *     resourcePossibleUseDTO, or with status {@code 400 (Bad Request)} if the resourcePossibleUse
   *     has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/resource-possible-uses")
  public ResponseEntity<ResourcePossibleUseDTO> createResourcePossibleUse(
      @RequestBody ResourcePossibleUseDTO resourcePossibleUseDTO) throws URISyntaxException {
    log.debug("REST request to save ResourcePossibleUse : {}", resourcePossibleUseDTO);
    if (resourcePossibleUseDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new resourcePossibleUse cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     resourcePossibleUseDTO, or with status {@code 400 (Bad Request)} if the
   *     resourcePossibleUseDTO is not valid, or with status {@code 500 (Internal Server Error)} if
   *     the resourcePossibleUseDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/resource-possible-uses")
  public ResponseEntity<ResourcePossibleUseDTO> updateResourcePossibleUse(
      @RequestBody ResourcePossibleUseDTO resourcePossibleUseDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, resourcePossibleUseDTO.getId().toString()))
        .body(result);
  }
   ...
   *     resourcePossibleUseDTO, or with status {@code 404 (Not Found)}.
   */
  ...
  public ResponseEntity<ResourcePossibleUseDTO> getResourcePossibleUse(@PathVariable Long id) {
    ...
    Optional<ResourcePossibleUseDTO> resourcePossibleUseDTO =
        resourcePossibleUseService.findOne(id);
