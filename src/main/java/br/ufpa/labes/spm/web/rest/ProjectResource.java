...
public class ProjectResource {
   ...
   *     projectDTO, or with status {@code 400 (Bad Request)} if the project has already an ID.
   ...
   *     projectDTO, or with status {@code 400 (Bad Request)} if the projectDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the projectDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/projects")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, projectDTO.getId().toString()))
        .body(result);
  }
