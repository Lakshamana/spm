...
public class JoinConResource {
   ...
   *     joinConDTO, or with status {@code 400 (Bad Request)} if the joinCon has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/join-cons")
  public ResponseEntity<JoinConDTO> createJoinCon(@RequestBody JoinConDTO joinConDTO)
   ...
   *     joinConDTO, or with status {@code 400 (Bad Request)} if the joinConDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the joinConDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/join-cons")
  public ResponseEntity<JoinConDTO> updateJoinCon(@RequestBody JoinConDTO joinConDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, joinConDTO.getId().toString()))
        .body(result);
  }
  ...
  public List<JoinConDTO> getAllJoinCons(
      @RequestParam(required = false) String filter,
      @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
    if ("themultipleconsuper-is-null".equals(filter)) {
      log.debug("REST request to get all JoinCons where theMultipleConSuper is null");
      return joinConService.findAllWhereTheMultipleConSuperIsNull();
    }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the joinConDTO,
