...
public class ToolDefinitionResource {
   ...
   *     toolDefinition, or with status {@code 400 (Bad Request)} if the toolDefinition has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/tool-definitions")
  public ResponseEntity<ToolDefinition> createToolDefinition(
      @RequestBody ToolDefinition toolDefinition) throws URISyntaxException {
   ...
   *     toolDefinition, or with status {@code 400 (Bad Request)} if the toolDefinition is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the toolDefinition couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/tool-definitions")
  public ResponseEntity<ToolDefinition> updateToolDefinition(
      @RequestBody ToolDefinition toolDefinition) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, toolDefinition.getId().toString()))
        .body(result);
  }
   ...
   *     toolDefinition, or with status {@code 404 (Not Found)}.
   */
  ...
  public ResponseEntity<ToolDefinition> getToolDefinition(@PathVariable Long id) {
    ...
    Optional<ToolDefinition> toolDefinition =
        toolDefinitionRepository.findOneWithEagerRelationships(id);
