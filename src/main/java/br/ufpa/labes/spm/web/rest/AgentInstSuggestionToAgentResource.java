...
public class AgentInstSuggestionToAgentResource {
  ...
  public AgentInstSuggestionToAgentResource(
      AgentInstSuggestionToAgentRepository agentInstSuggestionToAgentRepository) {
   ...
   *     agentInstSuggestionToAgent, or with status {@code 400 (Bad Request)} if the
   *     agentInstSuggestionToAgent has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/agent-inst-suggestion-to-agents")
  public ResponseEntity<AgentInstSuggestionToAgent> createAgentInstSuggestionToAgent(
      @RequestBody AgentInstSuggestionToAgent agentInstSuggestionToAgent)
    ...
    AgentInstSuggestionToAgent result =
        agentInstSuggestionToAgentRepository.save(agentInstSuggestionToAgent);
   ...
   *     agentInstSuggestionToAgent, or with status {@code 400 (Bad Request)} if the
   *     agentInstSuggestionToAgent is not valid, or with status {@code 500 (Internal Server Error)}
   *     if the agentInstSuggestionToAgent couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/agent-inst-suggestion-to-agents")
  public ResponseEntity<AgentInstSuggestionToAgent> updateAgentInstSuggestionToAgent(
      @RequestBody AgentInstSuggestionToAgent agentInstSuggestionToAgent)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, agentInstSuggestionToAgent.getId().toString()))
        .body(result);
  }
   ...
   *     agentInstSuggestionToAgent, or with status {@code 404 (Not Found)}.
   */
  @GetMapping("/agent-inst-suggestion-to-agents/{id}")
  public ResponseEntity<AgentInstSuggestionToAgent> getAgentInstSuggestionToAgent(
      @PathVariable Long id) {
    log.debug("REST request to get AgentInstSuggestionToAgent : {}", id);
    Optional<AgentInstSuggestionToAgent> agentInstSuggestionToAgent =
        agentInstSuggestionToAgentRepository.findById(id);
