...
public class RoleResource {
   ...
   *     role, or with status {@code 400 (Bad Request)} if the role has already an ID.
  ...
  public ResponseEntity<Role> createRole(@RequestBody Role role) throws URISyntaxException {
    ...
    if (role.getId() != null) {
      throw new BadRequestAlertException(
          "A new role cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     or with status {@code 400 (Bad Request)} if the role is not valid, or with status {@code
   *     500 (Internal Server Error)} if the role couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/roles")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, role.getId().toString()))
        .body(result);
  }
