...
public class LogEntryResource {
   ...
   *     logEntry, or with status {@code 400 (Bad Request)} if the logEntry has already an ID.
   ...
   *     logEntry, or with status {@code 400 (Bad Request)} if the logEntry is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the logEntry couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/log-entries")
  public ResponseEntity<LogEntry> updateLogEntry(@RequestBody LogEntry logEntry)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, logEntry.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the logEntry, or
