...
public class LogEntryResource {
   ...
   *     logEntryDTO, or with status {@code 400 (Bad Request)} if the logEntry has already an ID.
  ...
  public ResponseEntity<LogEntryDTO> createLogEntry(@RequestBody LogEntryDTO logEntryDTO)
    ...
    if (logEntryDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new logEntry cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     logEntryDTO, or with status {@code 400 (Bad Request)} if the logEntryDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the logEntryDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/log-entries")
  public ResponseEntity<LogEntryDTO> updateLogEntry(@RequestBody LogEntryDTO logEntryDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, logEntryDTO.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of logEntries in
   *     body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the logEntryDTO,
