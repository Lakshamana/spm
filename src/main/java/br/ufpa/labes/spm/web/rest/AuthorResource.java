...
public class AuthorResource {
   ...
   *     authorDTO, or with status {@code 400 (Bad Request)} if the author has already an ID.
  ...
  public ResponseEntity<AuthorDTO> createAuthor(@RequestBody AuthorDTO authorDTO)
    ...
    if (authorDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new author cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     authorDTO, or with status {@code 400 (Bad Request)} if the authorDTO is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the authorDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/authors")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, authorDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the authorDTO, or
