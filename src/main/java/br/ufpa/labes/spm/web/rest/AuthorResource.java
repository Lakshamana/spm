...
public class AuthorResource {
   ...
   *     author, or with status {@code 400 (Bad Request)} if the author has already an ID.
   ...
   *     author, or with status {@code 400 (Bad Request)} if the author is not valid, or with status
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, author.getId().toString()))
        .body(result);
  }
