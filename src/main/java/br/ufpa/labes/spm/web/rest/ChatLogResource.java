...
public class ChatLogResource {
   ...
   *     chatLogDTO, or with status {@code 400 (Bad Request)} if the chatLog has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/chat-logs")
  public ResponseEntity<ChatLogDTO> createChatLog(@RequestBody ChatLogDTO chatLogDTO)
      throws URISyntaxException {
    log.debug("REST request to save ChatLog : {}", chatLogDTO);
    if (chatLogDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new chatLog cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     chatLogDTO, or with status {@code 400 (Bad Request)} if the chatLogDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the chatLogDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, chatLogDTO.getId().toString()))
        .body(result);
  }
   ...
   *     many-to-many).
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of chatLogs in
   *     body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the chatLogDTO,
