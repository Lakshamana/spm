...
public class ResourceEventResource {
   ...
   *     resourceEventDTO, or with status {@code 400 (Bad Request)} if the resourceEvent has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/resource-events")
  public ResponseEntity<ResourceEventDTO> createResourceEvent(
      @RequestBody ResourceEventDTO resourceEventDTO) throws URISyntaxException {
    log.debug("REST request to save ResourceEvent : {}", resourceEventDTO);
    if (resourceEventDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new resourceEvent cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     resourceEventDTO, or with status {@code 400 (Bad Request)} if the resourceEventDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the resourceEventDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/resource-events")
  public ResponseEntity<ResourceEventDTO> updateResourceEvent(
      @RequestBody ResourceEventDTO resourceEventDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, resourceEventDTO.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of resourceEvents
   *     in body.
   */
   ...
   *     resourceEventDTO, or with status {@code 404 (Not Found)}.
   */
