...
public class ResourceInstSugResource {
   ...
   *     resourceInstSug, or with status {@code 400 (Bad Request)} if the resourceInstSug has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/resource-inst-sugs")
  public ResponseEntity<ResourceInstSug> createResourceInstSug(
      @RequestBody ResourceInstSug resourceInstSug) throws URISyntaxException {
    log.debug("REST request to save ResourceInstSug : {}", resourceInstSug);
    if (resourceInstSug.getId() != null) {
      throw new BadRequestAlertException(
          "A new resourceInstSug cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     resourceInstSug, or with status {@code 400 (Bad Request)} if the resourceInstSug is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the resourceInstSug couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/resource-inst-sugs")
  public ResponseEntity<ResourceInstSug> updateResourceInstSug(
      @RequestBody ResourceInstSug resourceInstSug) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, resourceInstSug.getId().toString()))
        .body(result);
  }
   ...
   *     resourceInstSugs in body.
   */
  @GetMapping("/resource-inst-sugs")
  public List<ResourceInstSug> getAllResourceInstSugs(
   ...
   *     resourceInstSug, or with status {@code 404 (Not Found)}.
   */
  ...
  public ResponseEntity<ResourceInstSug> getResourceInstSug(@PathVariable Long id) {
    ...
    Optional<ResourceInstSug> resourceInstSug =
        resourceInstSugRepository.findOneWithEagerRelationships(id);
