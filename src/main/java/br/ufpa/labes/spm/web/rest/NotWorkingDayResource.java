...
public class NotWorkingDayResource {
   ...
   *     notWorkingDay, or with status {@code 400 (Bad Request)} if the notWorkingDay has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/not-working-days")
  public ResponseEntity<NotWorkingDay> createNotWorkingDay(
      @Valid @RequestBody NotWorkingDay notWorkingDay) throws URISyntaxException {
    log.debug("REST request to save NotWorkingDay : {}", notWorkingDay);
    if (notWorkingDay.getId() != null) {
      throw new BadRequestAlertException(
          "A new notWorkingDay cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     notWorkingDay, or with status {@code 400 (Bad Request)} if the notWorkingDay is not valid,
   *     or with status {@code 500 (Internal Server Error)} if the notWorkingDay couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/not-working-days")
  public ResponseEntity<NotWorkingDay> updateNotWorkingDay(
      @Valid @RequestBody NotWorkingDay notWorkingDay) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, notWorkingDay.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of notWorkingDays
   *     in body.
   */
   ...
   *     notWorkingDay, or with status {@code 404 (Not Found)}.
   */
