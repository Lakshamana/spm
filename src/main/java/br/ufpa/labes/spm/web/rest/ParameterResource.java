...
public class ParameterResource {
   ...
   *     parameterDTO, or with status {@code 400 (Bad Request)} if the parameter has already an ID.
   ...
   *     parameterDTO, or with status {@code 400 (Bad Request)} if the parameterDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the parameterDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, parameterDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the parameterDTO,
