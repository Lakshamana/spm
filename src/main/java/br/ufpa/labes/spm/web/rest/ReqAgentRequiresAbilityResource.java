...
public class ReqAgentRequiresAbilityResource {
   ...
   *     reqAgentRequiresAbility, or with status {@code 400 (Bad Request)} if the
   *     reqAgentRequiresAbility has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/req-agent-requires-abilities")
  public ResponseEntity<ReqAgentRequiresAbility> createReqAgentRequiresAbility(
      @RequestBody ReqAgentRequiresAbility reqAgentRequiresAbility) throws URISyntaxException {
    ...
    ReqAgentRequiresAbility result =
        reqAgentRequiresAbilityRepository.save(reqAgentRequiresAbility);
   ...
   *     reqAgentRequiresAbility, or with status {@code 400 (Bad Request)} if the
   *     reqAgentRequiresAbility is not valid, or with status {@code 500 (Internal Server Error)} if
   *     the reqAgentRequiresAbility couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/req-agent-requires-abilities")
  public ResponseEntity<ReqAgentRequiresAbility> updateReqAgentRequiresAbility(
      @RequestBody ReqAgentRequiresAbility reqAgentRequiresAbility) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, reqAgentRequiresAbility.getId().toString()))
        .body(result);
  }
   ...
   *     reqAgentRequiresAbility, or with status {@code 404 (Not Found)}.
   */
  ...
  public ResponseEntity<ReqAgentRequiresAbility> getReqAgentRequiresAbility(@PathVariable Long id) {
    ...
    Optional<ReqAgentRequiresAbility> reqAgentRequiresAbility =
        reqAgentRequiresAbilityRepository.findById(id);
