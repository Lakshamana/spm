...
public class ReqAgentRequiresAbilityResource {
  ...
  public ReqAgentRequiresAbilityResource(
      ReqAgentRequiresAbilityService reqAgentRequiresAbilityService) {
   ...
   *     reqAgentRequiresAbilityDTO, or with status {@code 400 (Bad Request)} if the
   *     reqAgentRequiresAbility has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/req-agent-requires-abilities")
  public ResponseEntity<ReqAgentRequiresAbilityDTO> createReqAgentRequiresAbility(
      @RequestBody ReqAgentRequiresAbilityDTO reqAgentRequiresAbilityDTO)
    ...
    if (reqAgentRequiresAbilityDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new reqAgentRequiresAbility cannot already have an ID", ENTITY_NAME, "idexists");
    }
    ReqAgentRequiresAbilityDTO result =
        reqAgentRequiresAbilityService.save(reqAgentRequiresAbilityDTO);
   ...
   *     reqAgentRequiresAbilityDTO, or with status {@code 400 (Bad Request)} if the
   *     reqAgentRequiresAbilityDTO is not valid, or with status {@code 500 (Internal Server Error)}
   *     if the reqAgentRequiresAbilityDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/req-agent-requires-abilities")
  public ResponseEntity<ReqAgentRequiresAbilityDTO> updateReqAgentRequiresAbility(
      @RequestBody ReqAgentRequiresAbilityDTO reqAgentRequiresAbilityDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, reqAgentRequiresAbilityDTO.getId().toString()))
        .body(result);
  }
   ...
   *     reqAgentRequiresAbilities in body.
   */
   ...
   *     reqAgentRequiresAbilityDTO, or with status {@code 404 (Not Found)}.
   */
  @GetMapping("/req-agent-requires-abilities/{id}")
  public ResponseEntity<ReqAgentRequiresAbilityDTO> getReqAgentRequiresAbility(
      @PathVariable Long id) {
    log.debug("REST request to get ReqAgentRequiresAbility : {}", id);
    Optional<ReqAgentRequiresAbilityDTO> reqAgentRequiresAbilityDTO =
        reqAgentRequiresAbilityService.findOne(id);
