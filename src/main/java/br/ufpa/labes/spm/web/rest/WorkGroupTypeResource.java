...
public class WorkGroupTypeResource {
   ...
   *     workGroupTypeDTO, or with status {@code 400 (Bad Request)} if the workGroupType has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/work-group-types")
  public ResponseEntity<WorkGroupTypeDTO> createWorkGroupType(
      @RequestBody WorkGroupTypeDTO workGroupTypeDTO) throws URISyntaxException {
    log.debug("REST request to save WorkGroupType : {}", workGroupTypeDTO);
    if (workGroupTypeDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new workGroupType cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     workGroupTypeDTO, or with status {@code 400 (Bad Request)} if the workGroupTypeDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the workGroupTypeDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/work-group-types")
  public ResponseEntity<WorkGroupTypeDTO> updateWorkGroupType(
      @RequestBody WorkGroupTypeDTO workGroupTypeDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, workGroupTypeDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of workGroupTypes
   *     in body.
   */
  ...
  public List<WorkGroupTypeDTO> getAllWorkGroupTypes(
    ...
    if ("thetypesuper-is-null".equals(filter)) {
      log.debug("REST request to get all WorkGroupTypes where theTypeSuper is null");
      return workGroupTypeService.findAllWhereTheTypeSuperIsNull();
    }
   ...
   *     workGroupTypeDTO, or with status {@code 404 (Not Found)}.
   */
