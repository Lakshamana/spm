...
public class ArtifactTaskResource {
   ...
   *     artifactTaskDTO, or with status {@code 400 (Bad Request)} if the artifactTask has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/artifact-tasks")
  public ResponseEntity<ArtifactTaskDTO> createArtifactTask(
      @RequestBody ArtifactTaskDTO artifactTaskDTO) throws URISyntaxException {
   ...
   *     artifactTaskDTO, or with status {@code 400 (Bad Request)} if the artifactTaskDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the artifactTaskDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/artifact-tasks")
  public ResponseEntity<ArtifactTaskDTO> updateArtifactTask(
      @RequestBody ArtifactTaskDTO artifactTaskDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, artifactTaskDTO.getId().toString()))
        .body(result);
  }
   ...
   *     artifactTaskDTO, or with status {@code 404 (Not Found)}.
   */
