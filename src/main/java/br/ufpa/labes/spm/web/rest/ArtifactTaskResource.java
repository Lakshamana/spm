...
public class ArtifactTaskResource {
   ...
   *     artifactTask, or with status {@code 400 (Bad Request)} if the artifactTask has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/artifact-tasks")
  public ResponseEntity<ArtifactTask> createArtifactTask(@RequestBody ArtifactTask artifactTask)
      throws URISyntaxException {
    log.debug("REST request to save ArtifactTask : {}", artifactTask);
    if (artifactTask.getId() != null) {
      throw new BadRequestAlertException(
          "A new artifactTask cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     artifactTask, or with status {@code 400 (Bad Request)} if the artifactTask is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the artifactTask couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/artifact-tasks")
  public ResponseEntity<ArtifactTask> updateArtifactTask(@RequestBody ArtifactTask artifactTask)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, artifactTask.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of artifactTasks
   *     in body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the artifactTask,
