...
public class MetricDefinitionUnitResource {
   ...
   *     metricDefinitionUnitDTO, or with status {@code 400 (Bad Request)} if the
   *     metricDefinitionUnit has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/metric-definition-units")
  public ResponseEntity<MetricDefinitionUnitDTO> createMetricDefinitionUnit(
      @RequestBody MetricDefinitionUnitDTO metricDefinitionUnitDTO) throws URISyntaxException {
    log.debug("REST request to save MetricDefinitionUnit : {}", metricDefinitionUnitDTO);
    if (metricDefinitionUnitDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new metricDefinitionUnit cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     metricDefinitionUnitDTO, or with status {@code 400 (Bad Request)} if the
   *     metricDefinitionUnitDTO is not valid, or with status {@code 500 (Internal Server Error)} if
   *     the metricDefinitionUnitDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/metric-definition-units")
  public ResponseEntity<MetricDefinitionUnitDTO> updateMetricDefinitionUnit(
      @RequestBody MetricDefinitionUnitDTO metricDefinitionUnitDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, metricDefinitionUnitDTO.getId().toString()))
        .body(result);
  }
   ...
   *     metricDefinitionUnitDTO, or with status {@code 404 (Not Found)}.
   */
  ...
  public ResponseEntity<MetricDefinitionUnitDTO> getMetricDefinitionUnit(@PathVariable Long id) {
    ...
    Optional<MetricDefinitionUnitDTO> metricDefinitionUnitDTO =
        metricDefinitionUnitService.findOne(id);
