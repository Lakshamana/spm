...
public class ConsumableResource {
    ...
    public List<ConsumableDTO> getAllConsumables(@RequestParam(required = false) String filter) {
        if ("theresourcesuper-is-null".equals(filter)) {
            log.debug("REST request to get all Consumables where theResourceSuper is null");
            return consumableService.findAllWhereTheResourceSuperIsNull();
        }
