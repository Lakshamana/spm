...
public class RequiredResourceResource {
   ...
   *     requiredResourceDTO, or with status {@code 400 (Bad Request)} if the requiredResource has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/required-resources")
  public ResponseEntity<RequiredResourceDTO> createRequiredResource(
      @RequestBody RequiredResourceDTO requiredResourceDTO) throws URISyntaxException {
    log.debug("REST request to save RequiredResource : {}", requiredResourceDTO);
    if (requiredResourceDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new requiredResource cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     requiredResourceDTO, or with status {@code 400 (Bad Request)} if the requiredResourceDTO is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the requiredResourceDTO
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/required-resources")
  public ResponseEntity<RequiredResourceDTO> updateRequiredResource(
      @RequestBody RequiredResourceDTO requiredResourceDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, requiredResourceDTO.getId().toString()))
        .body(result);
  }
   ...
   *     requiredResourceDTO, or with status {@code 404 (Not Found)}.
   */
