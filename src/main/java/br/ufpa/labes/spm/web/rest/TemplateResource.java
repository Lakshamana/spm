...
public class TemplateResource {
   ...
   *     templateDTO, or with status {@code 400 (Bad Request)} if the template has already an ID.
   ...
   *     templateDTO, or with status {@code 400 (Bad Request)} if the templateDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the templateDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, templateDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the templateDTO,
