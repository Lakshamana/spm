...
public class ToolParameterResource {
   ...
   *     toolParameter, or with status {@code 400 (Bad Request)} if the toolParameter has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/tool-parameters")
  public ResponseEntity<ToolParameter> createToolParameter(@RequestBody ToolParameter toolParameter)
      throws URISyntaxException {
    log.debug("REST request to save ToolParameter : {}", toolParameter);
    if (toolParameter.getId() != null) {
      throw new BadRequestAlertException(
          "A new toolParameter cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     toolParameter, or with status {@code 400 (Bad Request)} if the toolParameter is not valid,
   *     or with status {@code 500 (Internal Server Error)} if the toolParameter couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/tool-parameters")
  public ResponseEntity<ToolParameter> updateToolParameter(@RequestBody ToolParameter toolParameter)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, toolParameter.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of toolParameters
   *     in body.
   */
   ...
   *     toolParameter, or with status {@code 404 (Not Found)}.
   */
