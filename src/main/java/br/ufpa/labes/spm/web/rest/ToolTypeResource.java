...
public class ToolTypeResource {
   ...
   *     toolType, or with status {@code 400 (Bad Request)} if the toolType has already an ID.
   ...
   *     toolType, or with status {@code 400 (Bad Request)} if the toolType is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the toolType couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/tool-types")
  public ResponseEntity<ToolType> updateToolType(@RequestBody ToolType toolType)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, toolType.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the toolType, or
