...
public class TypeResource {
   ...
   *     type, or with status {@code 400 (Bad Request)} if the type has already an ID.
   ...
   *     or with status {@code 400 (Bad Request)} if the type is not valid, or with status {@code
   *     500 (Internal Server Error)} if the type couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/types")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, type.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the type, or with
