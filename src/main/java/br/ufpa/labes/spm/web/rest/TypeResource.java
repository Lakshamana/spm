...
public class TypeResource {
   ...
   *     typeDTO, or with status {@code 400 (Bad Request)} if the type has already an ID.
  ...
  public ResponseEntity<TypeDTO> createType(@RequestBody TypeDTO typeDTO)
    ...
    if (typeDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new type cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     typeDTO, or with status {@code 400 (Bad Request)} if the typeDTO is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the typeDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/types")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, typeDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the typeDTO, or
