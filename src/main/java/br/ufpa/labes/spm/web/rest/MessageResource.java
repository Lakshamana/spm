...
public class MessageResource {
   ...
   *     messageDTO, or with status {@code 400 (Bad Request)} if the message has already an ID.
   ...
   *     messageDTO, or with status {@code 400 (Bad Request)} if the messageDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the messageDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/messages")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, messageDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the messageDTO,
