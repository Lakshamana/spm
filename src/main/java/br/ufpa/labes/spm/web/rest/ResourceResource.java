...
public class ResourceResource {
   ...
   *     resource, or with status {@code 400 (Bad Request)} if the resource has already an ID.
  ...
  public ResponseEntity<Resource> createResource(@RequestBody Resource resource)
    ...
    if (resource.getId() != null) {
      throw new BadRequestAlertException(
          "A new resource cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     resource, or with status {@code 400 (Bad Request)} if the resource is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the resource couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, resource.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the resource, or
