...
public class ProcessModelEventResource {
   ...
   *     processModelEvent, or with status {@code 400 (Bad Request)} if the processModelEvent has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/process-model-events")
  public ResponseEntity<ProcessModelEvent> createProcessModelEvent(
      @RequestBody ProcessModelEvent processModelEvent) throws URISyntaxException {
   ...
   *     processModelEvent, or with status {@code 400 (Bad Request)} if the processModelEvent is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the processModelEvent couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/process-model-events")
  public ResponseEntity<ProcessModelEvent> updateProcessModelEvent(
      @RequestBody ProcessModelEvent processModelEvent) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, processModelEvent.getId().toString()))
        .body(result);
  }
   ...
   *     processModelEvent, or with status {@code 404 (Not Found)}.
   */
