...
public class ProcessModelEventResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new processModelEventDTO, or with status {@code 400 (Bad Request)} if the processModelEvent has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/process-model-events")
    public ResponseEntity<ProcessModelEventDTO> createProcessModelEvent(@RequestBody ProcessModelEventDTO processModelEventDTO) throws URISyntaxException {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated processModelEventDTO,
     * or with status {@code 400 (Bad Request)} if the processModelEventDTO is not valid,
     * or with status {@code 500 (Internal Server Error)} if the processModelEventDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/process-model-events")
    public ResponseEntity<ProcessModelEventDTO> updateProcessModelEvent(@RequestBody ProcessModelEventDTO processModelEventDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, processModelEventDTO.getId().toString()))
            .body(result);
    }
    ...
    public List<ProcessModelEventDTO> getAllProcessModelEvents(@RequestParam(required = false) String filter) {
        if ("theeventsuper-is-null".equals(filter)) {
            log.debug("REST request to get all ProcessModelEvents where theEventSuper is null");
            return processModelEventService.findAllWhereTheEventSuperIsNull();
        }
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the processModelEventDTO, or with status {@code 404 (Not Found)}.
     */
