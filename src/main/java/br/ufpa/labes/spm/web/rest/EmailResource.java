...
public class EmailResource {
   ...
   *     emailDTO, or with status {@code 400 (Bad Request)} if the email has already an ID.
  ...
  public ResponseEntity<EmailDTO> createEmail(@RequestBody EmailDTO emailDTO)
    ...
    if (emailDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new email cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     emailDTO, or with status {@code 400 (Bad Request)} if the emailDTO is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the emailDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/emails")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, emailDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the emailDTO, or
