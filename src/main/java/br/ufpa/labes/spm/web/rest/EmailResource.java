...
public class EmailResource {
   ...
   *     email, or with status {@code 400 (Bad Request)} if the email has already an ID.
   ...
   *     email, or with status {@code 400 (Bad Request)} if the email is not valid, or with status
   *     {@code 500 (Internal Server Error)} if the email couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/emails")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, email.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the email, or
