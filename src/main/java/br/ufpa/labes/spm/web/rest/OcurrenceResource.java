...
public class OcurrenceResource {
   ...
   *     ocurrence, or with status {@code 400 (Bad Request)} if the ocurrence has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/ocurrences")
  public ResponseEntity<Ocurrence> createOcurrence(@RequestBody Ocurrence ocurrence)
   ...
   *     ocurrence, or with status {@code 400 (Bad Request)} if the ocurrence is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the ocurrence couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/ocurrences")
  public ResponseEntity<Ocurrence> updateOcurrence(@RequestBody Ocurrence ocurrence)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, ocurrence.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the ocurrence, or
