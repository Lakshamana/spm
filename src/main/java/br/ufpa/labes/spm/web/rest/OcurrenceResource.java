...
public class OcurrenceResource {
   ...
   *     ocurrenceDTO, or with status {@code 400 (Bad Request)} if the ocurrence has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/ocurrences")
  public ResponseEntity<OcurrenceDTO> createOcurrence(@RequestBody OcurrenceDTO ocurrenceDTO)
      throws URISyntaxException {
    log.debug("REST request to save Ocurrence : {}", ocurrenceDTO);
    if (ocurrenceDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new ocurrence cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     ocurrenceDTO, or with status {@code 400 (Bad Request)} if the ocurrenceDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the ocurrenceDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/ocurrences")
  public ResponseEntity<OcurrenceDTO> updateOcurrence(@RequestBody OcurrenceDTO ocurrenceDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, ocurrenceDTO.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of ocurrences in
   *     body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the ocurrenceDTO,
