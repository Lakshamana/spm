...
public class ActivityTypeResource {
   ...
   *     activityTypeDTO, or with status {@code 400 (Bad Request)} if the activityType has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/activity-types")
  public ResponseEntity<ActivityTypeDTO> createActivityType(
      @RequestBody ActivityTypeDTO activityTypeDTO) throws URISyntaxException {
    log.debug("REST request to save ActivityType : {}", activityTypeDTO);
    if (activityTypeDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new activityType cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     activityTypeDTO, or with status {@code 400 (Bad Request)} if the activityTypeDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the activityTypeDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, activityTypeDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of activityTypes
   *     in body.
   */
  @GetMapping("/activity-types")
  public List<ActivityTypeDTO> getAllActivityTypes(@RequestParam(required = false) String filter) {
    if ("thetypesuper-is-null".equals(filter)) {
      log.debug("REST request to get all ActivityTypes where theTypeSuper is null");
      return activityTypeService.findAllWhereTheTypeSuperIsNull();
    }
   ...
   *     activityTypeDTO, or with status {@code 404 (Not Found)}.
   */
