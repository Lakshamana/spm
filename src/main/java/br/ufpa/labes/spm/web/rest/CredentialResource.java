...
public class CredentialResource {
   ...
   *     credentialDTO, or with status {@code 400 (Bad Request)} if the credential has already an
   *     ID.
   ...
   *     credentialDTO, or with status {@code 400 (Bad Request)} if the credentialDTO is not valid,
   *     or with status {@code 500 (Internal Server Error)} if the credentialDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, credentialDTO.getId().toString()))
        .body(result);
  }
   ...
   *     credentialDTO, or with status {@code 404 (Not Found)}.
   */
