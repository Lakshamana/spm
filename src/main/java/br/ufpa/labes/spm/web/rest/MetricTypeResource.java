...
public class MetricTypeResource {
   ...
   *     metricType, or with status {@code 400 (Bad Request)} if the metricType has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/metric-types")
  public ResponseEntity<MetricType> createMetricType(@RequestBody MetricType metricType)
   ...
   *     metricType, or with status {@code 400 (Bad Request)} if the metricType is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the metricType couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/metric-types")
  public ResponseEntity<MetricType> updateMetricType(@RequestBody MetricType metricType)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, metricType.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the metricType,
