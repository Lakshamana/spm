...
public class BranchConResource {
    ...
    public List<BranchConDTO> getAllBranchCons(@RequestParam(required = false) String filter) {
        if ("themultipleconsuper-is-null".equals(filter)) {
            log.debug("REST request to get all BranchCons where theMultipleConSuper is null");
            return branchConService.findAllWhereTheMultipleConSuperIsNull();
        }
