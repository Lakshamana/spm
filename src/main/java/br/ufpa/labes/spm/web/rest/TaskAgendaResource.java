...
public class TaskAgendaResource {
   ...
   *     taskAgenda, or with status {@code 400 (Bad Request)} if the taskAgenda has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/task-agenda")
  public ResponseEntity<TaskAgenda> createTaskAgenda(@RequestBody TaskAgenda taskAgenda)
   ...
   *     taskAgenda, or with status {@code 400 (Bad Request)} if the taskAgenda is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the taskAgenda couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/task-agenda")
  public ResponseEntity<TaskAgenda> updateTaskAgenda(@RequestBody TaskAgenda taskAgenda)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, taskAgenda.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the taskAgenda,
