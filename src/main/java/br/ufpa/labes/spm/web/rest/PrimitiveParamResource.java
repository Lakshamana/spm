...
public class PrimitiveParamResource {
   ...
   *     primitiveParam, or with status {@code 400 (Bad Request)} if the primitiveParam has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/primitive-params")
  public ResponseEntity<PrimitiveParam> createPrimitiveParam(
      @RequestBody PrimitiveParam primitiveParam) throws URISyntaxException {
   ...
   *     primitiveParam, or with status {@code 400 (Bad Request)} if the primitiveParam is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the primitiveParam couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/primitive-params")
  public ResponseEntity<PrimitiveParam> updatePrimitiveParam(
      @RequestBody PrimitiveParam primitiveParam) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, primitiveParam.getId().toString()))
        .body(result);
  }
   ...
   *     primitiveParam, or with status {@code 404 (Not Found)}.
   */
