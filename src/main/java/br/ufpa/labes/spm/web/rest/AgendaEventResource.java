...
public class AgendaEventResource {
   ...
   *     agendaEventDTO, or with status {@code 400 (Bad Request)} if the agendaEvent has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/agenda-events")
  public ResponseEntity<AgendaEventDTO> createAgendaEvent(
      @RequestBody AgendaEventDTO agendaEventDTO) throws URISyntaxException {
    log.debug("REST request to save AgendaEvent : {}", agendaEventDTO);
    if (agendaEventDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new agendaEvent cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     agendaEventDTO, or with status {@code 400 (Bad Request)} if the agendaEventDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the agendaEventDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/agenda-events")
  public ResponseEntity<AgendaEventDTO> updateAgendaEvent(
      @RequestBody AgendaEventDTO agendaEventDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, agendaEventDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of agendaEvents in
   *     body.
   */
  @GetMapping("/agenda-events")
  public List<AgendaEventDTO> getAllAgendaEvents(@RequestParam(required = false) String filter) {
    if ("theeventsuper-is-null".equals(filter)) {
      log.debug("REST request to get all AgendaEvents where theEventSuper is null");
      return agendaEventService.findAllWhereTheEventSuperIsNull();
    }
   ...
   *     agendaEventDTO, or with status {@code 404 (Not Found)}.
   */
