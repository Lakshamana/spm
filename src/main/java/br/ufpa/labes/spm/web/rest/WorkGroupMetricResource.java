...
public class WorkGroupMetricResource {
   ...
   *     workGroupMetricDTO, or with status {@code 400 (Bad Request)} if the workGroupMetric has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/work-group-metrics")
  public ResponseEntity<WorkGroupMetricDTO> createWorkGroupMetric(
      @RequestBody WorkGroupMetricDTO workGroupMetricDTO) throws URISyntaxException {
    log.debug("REST request to save WorkGroupMetric : {}", workGroupMetricDTO);
    if (workGroupMetricDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new workGroupMetric cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     workGroupMetricDTO, or with status {@code 400 (Bad Request)} if the workGroupMetricDTO is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the workGroupMetricDTO
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/work-group-metrics")
  public ResponseEntity<WorkGroupMetricDTO> updateWorkGroupMetric(
      @RequestBody WorkGroupMetricDTO workGroupMetricDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, workGroupMetricDTO.getId().toString()))
        .body(result);
  }
   ...
   *     workGroupMetrics in body.
   */
  @GetMapping("/work-group-metrics")
  public List<WorkGroupMetricDTO> getAllWorkGroupMetrics() {
   ...
   *     workGroupMetricDTO, or with status {@code 404 (Not Found)}.
   */
