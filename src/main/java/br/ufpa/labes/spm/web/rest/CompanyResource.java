...
public class CompanyResource {
   ...
   *     company, or with status {@code 400 (Bad Request)} if the company has already an ID.
   ...
   *     company, or with status {@code 400 (Bad Request)} if the company is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the company couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, company.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the company, or
