...
public class TagStatsResource {
   ...
   *     tagStats, or with status {@code 400 (Bad Request)} if the tagStats has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/tag-stats")
  public ResponseEntity<TagStats> createTagStats(@RequestBody TagStats tagStats)
   ...
   *     tagStats, or with status {@code 400 (Bad Request)} if the tagStats is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the tagStats couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/tag-stats")
  public ResponseEntity<TagStats> updateTagStats(@RequestBody TagStats tagStats)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, tagStats.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the tagStats, or
