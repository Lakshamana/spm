...
public class ProcessEventResource {
   ...
   *     processEventDTO, or with status {@code 400 (Bad Request)} if the processEvent has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/process-events")
  public ResponseEntity<ProcessEventDTO> createProcessEvent(
      @RequestBody ProcessEventDTO processEventDTO) throws URISyntaxException {
    log.debug("REST request to save ProcessEvent : {}", processEventDTO);
    if (processEventDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new processEvent cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     processEventDTO, or with status {@code 400 (Bad Request)} if the processEventDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the processEventDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/process-events")
  public ResponseEntity<ProcessEventDTO> updateProcessEvent(
      @RequestBody ProcessEventDTO processEventDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, processEventDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of processEvents
   *     in body.
   */
  @GetMapping("/process-events")
  public List<ProcessEventDTO> getAllProcessEvents(@RequestParam(required = false) String filter) {
    if ("theeventsuper-is-null".equals(filter)) {
      log.debug("REST request to get all ProcessEvents where theEventSuper is null");
      return processEventService.findAllWhereTheEventSuperIsNull();
    }
   ...
   *     processEventDTO, or with status {@code 404 (Not Found)}.
   */
