...
public class SpmConfigurationResource {
   ...
   *     spmConfigurationDTO, or with status {@code 400 (Bad Request)} if the spmConfiguration has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/spm-configurations")
  public ResponseEntity<SpmConfigurationDTO> createSpmConfiguration(
      @RequestBody SpmConfigurationDTO spmConfigurationDTO) throws URISyntaxException {
    log.debug("REST request to save SpmConfiguration : {}", spmConfigurationDTO);
    if (spmConfigurationDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new spmConfiguration cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     spmConfigurationDTO, or with status {@code 400 (Bad Request)} if the spmConfigurationDTO is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the spmConfigurationDTO
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/spm-configurations")
  public ResponseEntity<SpmConfigurationDTO> updateSpmConfiguration(
      @RequestBody SpmConfigurationDTO spmConfigurationDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, spmConfigurationDTO.getId().toString()))
        .body(result);
  }
   ...
   *     spmConfigurations in body.
   */
  @GetMapping("/spm-configurations")
  public List<SpmConfigurationDTO> getAllSpmConfigurations(
   ...
   *     spmConfigurationDTO, or with status {@code 404 (Not Found)}.
   */
