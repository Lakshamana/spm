...
public class ConnectionResource {
   ...
   *     connectionDTO, or with status {@code 400 (Bad Request)} if the connection has already an
   *     ID.
   ...
   *     connectionDTO, or with status {@code 400 (Bad Request)} if the connectionDTO is not valid,
   *     or with status {@code 500 (Internal Server Error)} if the connectionDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/connections")
  public ResponseEntity<ConnectionDTO> updateConnection(@RequestBody ConnectionDTO connectionDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, connectionDTO.getId().toString()))
        .body(result);
  }
   ...
   *     connectionDTO, or with status {@code 404 (Not Found)}.
   */
