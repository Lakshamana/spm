...
public class SubroutineResource {
   ...
   *     subroutineDTO, or with status {@code 400 (Bad Request)} if the subroutine has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/subroutines")
  public ResponseEntity<SubroutineDTO> createSubroutine(@RequestBody SubroutineDTO subroutineDTO)
   ...
   *     subroutineDTO, or with status {@code 400 (Bad Request)} if the subroutineDTO is not valid,
   *     or with status {@code 500 (Internal Server Error)} if the subroutineDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/subroutines")
  public ResponseEntity<SubroutineDTO> updateSubroutine(@RequestBody SubroutineDTO subroutineDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, subroutineDTO.getId().toString()))
        .body(result);
  }
   ...
   *     subroutineDTO, or with status {@code 404 (Not Found)}.
   */
