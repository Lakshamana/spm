...
public class TagResource {
   ...
   *     or with status {@code 400 (Bad Request)} if the tag has already an ID.
   ...
   *     or with status {@code 400 (Bad Request)} if the tag is not valid, or with status {@code 500
   *     (Internal Server Error)} if the tag couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/tags")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, tag.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the tag, or with
