...
public class PluginResource {
   ...
   *     plugin, or with status {@code 400 (Bad Request)} if the plugin has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/plugins")
  public ResponseEntity<Plugin> createPlugin(@Valid @RequestBody Plugin plugin)
      throws URISyntaxException {
    log.debug("REST request to save Plugin : {}", plugin);
    if (plugin.getId() != null) {
      throw new BadRequestAlertException(
          "A new plugin cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     plugin, or with status {@code 400 (Bad Request)} if the plugin is not valid, or with status
   *     {@code 500 (Internal Server Error)} if the plugin couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/plugins")
  public ResponseEntity<Plugin> updatePlugin(@Valid @RequestBody Plugin plugin)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, plugin.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of plugins in
   *     body.
   */
  @GetMapping("/plugins")
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the plugin, or
