...
public class PluginResource {
   ...
   *     pluginDTO, or with status {@code 400 (Bad Request)} if the plugin has already an ID.
   ...
   *     pluginDTO, or with status {@code 400 (Bad Request)} if the pluginDTO is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the pluginDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/plugins")
  public ResponseEntity<PluginDTO> updatePlugin(@Valid @RequestBody PluginDTO pluginDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, pluginDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the pluginDTO, or
