...
public class ArtifactMetricResource {
    ...
    public List<ArtifactMetricDTO> getAllArtifactMetrics(@RequestParam(required = false) String filter) {
        if ("themetricsuper-is-null".equals(filter)) {
            log.debug("REST request to get all ArtifactMetrics where theMetricSuper is null");
            return artifactMetricService.findAllWhereTheMetricSuperIsNull();
        }
