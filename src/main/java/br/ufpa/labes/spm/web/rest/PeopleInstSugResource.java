...
public class PeopleInstSugResource {
   ...
   *     peopleInstSugDTO, or with status {@code 400 (Bad Request)} if the peopleInstSug has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/people-inst-sugs")
  public ResponseEntity<PeopleInstSugDTO> createPeopleInstSug(
      @RequestBody PeopleInstSugDTO peopleInstSugDTO) throws URISyntaxException {
    log.debug("REST request to save PeopleInstSug : {}", peopleInstSugDTO);
    if (peopleInstSugDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new peopleInstSug cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     peopleInstSugDTO, or with status {@code 400 (Bad Request)} if the peopleInstSugDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the peopleInstSugDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/people-inst-sugs")
  public ResponseEntity<PeopleInstSugDTO> updatePeopleInstSug(
      @RequestBody PeopleInstSugDTO peopleInstSugDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, peopleInstSugDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of peopleInstSugs
   *     in body.
   */
   ...
   *     peopleInstSugDTO, or with status {@code 404 (Not Found)}.
   */
