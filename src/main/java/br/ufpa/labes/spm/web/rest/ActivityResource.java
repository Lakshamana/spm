...
public class ActivityResource {
   ...
   *     activityDTO, or with status {@code 400 (Bad Request)} if the activity has already an ID.
   ...
   *     activityDTO, or with status {@code 400 (Bad Request)} if the activityDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the activityDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, activityDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the activityDTO,
