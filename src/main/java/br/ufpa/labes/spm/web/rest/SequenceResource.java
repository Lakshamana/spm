...
public class SequenceResource {
   ...
   *     sequenceDTO, or with status {@code 400 (Bad Request)} if the sequence has already an ID.
   ...
   *     sequenceDTO, or with status {@code 400 (Bad Request)} if the sequenceDTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the sequenceDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/sequences")
  public ResponseEntity<SequenceDTO> updateSequence(@RequestBody SequenceDTO sequenceDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, sequenceDTO.getId().toString()))
        .body(result);
  }
  ...
  public List<SequenceDTO> getAllSequences(@RequestParam(required = false) String filter) {
    if ("thesimpleconsuper-is-null".equals(filter)) {
      log.debug("REST request to get all Sequences where theSimpleConSuper is null");
      return sequenceService.findAllWhereTheSimpleConSuperIsNull();
    }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the sequenceDTO,
