...
public class WorkGroupEstimationResource {
   ...
   *     workGroupEstimationDTO, or with status {@code 400 (Bad Request)} if the workGroupEstimation
   *     has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/work-group-estimations")
  public ResponseEntity<WorkGroupEstimationDTO> createWorkGroupEstimation(
      @RequestBody WorkGroupEstimationDTO workGroupEstimationDTO) throws URISyntaxException {
    log.debug("REST request to save WorkGroupEstimation : {}", workGroupEstimationDTO);
    if (workGroupEstimationDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new workGroupEstimation cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     workGroupEstimationDTO, or with status {@code 400 (Bad Request)} if the
   *     workGroupEstimationDTO is not valid, or with status {@code 500 (Internal Server Error)} if
   *     the workGroupEstimationDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/work-group-estimations")
  public ResponseEntity<WorkGroupEstimationDTO> updateWorkGroupEstimation(
      @RequestBody WorkGroupEstimationDTO workGroupEstimationDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, workGroupEstimationDTO.getId().toString()))
        .body(result);
  }
  ...
  public List<WorkGroupEstimationDTO> getAllWorkGroupEstimations(
    ...
    if ("theestimationsuper-is-null".equals(filter)) {
      log.debug("REST request to get all WorkGroupEstimations where theEstimationSuper is null");
      return workGroupEstimationService.findAllWhereTheEstimationSuperIsNull();
    }
   ...
   *     workGroupEstimationDTO, or with status {@code 404 (Not Found)}.
   */
  ...
  public ResponseEntity<WorkGroupEstimationDTO> getWorkGroupEstimation(@PathVariable Long id) {
    ...
    Optional<WorkGroupEstimationDTO> workGroupEstimationDTO =
        workGroupEstimationService.findOne(id);
