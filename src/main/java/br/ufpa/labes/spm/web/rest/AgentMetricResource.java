...
public class AgentMetricResource {
   ...
   *     agentMetricDTO, or with status {@code 400 (Bad Request)} if the agentMetric has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/agent-metrics")
  public ResponseEntity<AgentMetricDTO> createAgentMetric(
      @RequestBody AgentMetricDTO agentMetricDTO) throws URISyntaxException {
    log.debug("REST request to save AgentMetric : {}", agentMetricDTO);
    if (agentMetricDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new agentMetric cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     agentMetricDTO, or with status {@code 400 (Bad Request)} if the agentMetricDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the agentMetricDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/agent-metrics")
  public ResponseEntity<AgentMetricDTO> updateAgentMetric(
      @RequestBody AgentMetricDTO agentMetricDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, agentMetricDTO.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of agentMetrics in
   *     body.
   */
  @GetMapping("/agent-metrics")
  public List<AgentMetricDTO> getAllAgentMetrics() {
   ...
   *     agentMetricDTO, or with status {@code 404 (Not Found)}.
   */
