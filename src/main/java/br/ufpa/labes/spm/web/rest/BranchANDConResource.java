...
public class BranchANDConResource {
   ...
   *     branchANDConDTO, or with status {@code 400 (Bad Request)} if the branchANDCon has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/branch-and-cons")
  public ResponseEntity<BranchANDConDTO> createBranchANDCon(
   ...
   *     branchANDConDTO, or with status {@code 400 (Bad Request)} if the branchANDConDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the branchANDConDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/branch-and-cons")
  public ResponseEntity<BranchANDConDTO> updateBranchANDCon(
      @RequestBody BranchANDConDTO branchANDConDTO) throws URISyntaxException {
  ...
  public List<BranchANDConDTO> getAllBranchANDCons(
      @RequestParam(required = false) String filter,
      @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
    if ("thebranchconsuper-is-null".equals(filter)) {
      log.debug("REST request to get all BranchANDCons where theBranchConSuper is null");
      return branchANDConService.findAllWhereTheBranchConSuperIsNull();
    }
   ...
   *     branchANDConDTO, or with status {@code 404 (Not Found)}.
   */
