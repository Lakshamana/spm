...
public class ActivityEstimationResource {
   ...
   *     activityEstimationDTO, or with status {@code 400 (Bad Request)} if the activityEstimation
   *     has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/activity-estimations")
  public ResponseEntity<ActivityEstimationDTO> createActivityEstimation(
      @RequestBody ActivityEstimationDTO activityEstimationDTO) throws URISyntaxException {
    log.debug("REST request to save ActivityEstimation : {}", activityEstimationDTO);
    if (activityEstimationDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new activityEstimation cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     activityEstimationDTO, or with status {@code 400 (Bad Request)} if the
   *     activityEstimationDTO is not valid, or with status {@code 500 (Internal Server Error)} if
   *     the activityEstimationDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/activity-estimations")
  public ResponseEntity<ActivityEstimationDTO> updateActivityEstimation(
      @RequestBody ActivityEstimationDTO activityEstimationDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, activityEstimationDTO.getId().toString()))
        .body(result);
  }
  ...
  public List<ActivityEstimationDTO> getAllActivityEstimations(
    ...
    if ("theestimationsuper-is-null".equals(filter)) {
      log.debug("REST request to get all ActivityEstimations where theEstimationSuper is null");
      return activityEstimationService.findAllWhereTheEstimationSuperIsNull();
    }
   ...
   *     activityEstimationDTO, or with status {@code 404 (Not Found)}.
   */
