...
public class ActivityEstimationResource {
   ...
   *     activityEstimation, or with status {@code 400 (Bad Request)} if the activityEstimation has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/activity-estimations")
  public ResponseEntity<ActivityEstimation> createActivityEstimation(
      @RequestBody ActivityEstimation activityEstimation) throws URISyntaxException {
   ...
   *     activityEstimation, or with status {@code 400 (Bad Request)} if the activityEstimation is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the activityEstimation
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/activity-estimations")
  public ResponseEntity<ActivityEstimation> updateActivityEstimation(
      @RequestBody ActivityEstimation activityEstimation) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, activityEstimation.getId().toString()))
        .body(result);
  }
   ...
   *     activityEstimation, or with status {@code 404 (Not Found)}.
   */
