...
public class PlainResource {
   ...
   *     plainDTO, or with status {@code 400 (Bad Request)} if the plain has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/plains")
  public ResponseEntity<PlainDTO> createPlain(@RequestBody PlainDTO plainDTO)
   ...
   *     plainDTO, or with status {@code 400 (Bad Request)} if the plainDTO is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the plainDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/plains")
  public ResponseEntity<PlainDTO> updatePlain(@RequestBody PlainDTO plainDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, plainDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the plainDTO, or
