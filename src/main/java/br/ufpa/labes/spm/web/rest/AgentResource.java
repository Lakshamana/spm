...
public class AgentResource {
   ...
   *     agentDTO, or with status {@code 400 (Bad Request)} if the agent has already an ID.
   ...
   *     agentDTO, or with status {@code 400 (Bad Request)} if the agentDTO is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the agentDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/agents")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, agentDTO.getId().toString()))
        .body(result);
  }
  ...
  public List<AgentDTO> getAllAgents(
    ...
    if ("thechatmessage-is-null".equals(filter)) {
      ...
      return agentService.findAllWhereTheChatMessageIsNull();
    }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the agentDTO, or
