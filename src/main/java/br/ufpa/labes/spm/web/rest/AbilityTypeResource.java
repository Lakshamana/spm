...
public class AbilityTypeResource {
   ...
   *     abilityTypeDTO, or with status {@code 400 (Bad Request)} if the abilityType has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/ability-types")
  public ResponseEntity<AbilityTypeDTO> createAbilityType(
      @RequestBody AbilityTypeDTO abilityTypeDTO) throws URISyntaxException {
   ...
   *     abilityTypeDTO, or with status {@code 400 (Bad Request)} if the abilityTypeDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the abilityTypeDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/ability-types")
  public ResponseEntity<AbilityTypeDTO> updateAbilityType(
      @RequestBody AbilityTypeDTO abilityTypeDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, abilityTypeDTO.getId().toString()))
        .body(result);
  }
  ...
  public List<AbilityTypeDTO> getAllAbilityTypes(@RequestParam(required = false) String filter) {
    if ("thetypesuper-is-null".equals(filter)) {
      log.debug("REST request to get all AbilityTypes where theTypeSuper is null");
      return abilityTypeService.findAllWhereTheTypeSuperIsNull();
    }
   ...
   *     abilityTypeDTO, or with status {@code 404 (Not Found)}.
   */
