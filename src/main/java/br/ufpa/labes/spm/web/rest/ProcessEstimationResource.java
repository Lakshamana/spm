...
public class ProcessEstimationResource {
   ...
   *     processEstimationDTO, or with status {@code 400 (Bad Request)} if the processEstimation has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/process-estimations")
  public ResponseEntity<ProcessEstimationDTO> createProcessEstimation(
      @RequestBody ProcessEstimationDTO processEstimationDTO) throws URISyntaxException {
    log.debug("REST request to save ProcessEstimation : {}", processEstimationDTO);
    if (processEstimationDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new processEstimation cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     processEstimationDTO, or with status {@code 400 (Bad Request)} if the processEstimationDTO
   *     is not valid, or with status {@code 500 (Internal Server Error)} if the
   *     processEstimationDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/process-estimations")
  public ResponseEntity<ProcessEstimationDTO> updateProcessEstimation(
      @RequestBody ProcessEstimationDTO processEstimationDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, processEstimationDTO.getId().toString()))
        .body(result);
  }
  ...
  public List<ProcessEstimationDTO> getAllProcessEstimations(
    ...
    if ("theestimationsuper-is-null".equals(filter)) {
      log.debug("REST request to get all ProcessEstimations where theEstimationSuper is null");
      return processEstimationService.findAllWhereTheEstimationSuperIsNull();
    }
   ...
   *     processEstimationDTO, or with status {@code 404 (Not Found)}.
   */
