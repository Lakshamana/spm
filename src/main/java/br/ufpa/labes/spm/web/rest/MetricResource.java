...
public class MetricResource {
   ...
   *     metric, or with status {@code 400 (Bad Request)} if the metric has already an ID.
   ...
   *     metric, or with status {@code 400 (Bad Request)} if the metric is not valid, or with status
   *     {@code 500 (Internal Server Error)} if the metric couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/metrics")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, metric.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the metric, or
