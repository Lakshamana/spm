...
public class AgentPlaysRoleResource {
   ...
   *     agentPlaysRole, or with status {@code 400 (Bad Request)} if the agentPlaysRole has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/agent-plays-roles")
  public ResponseEntity<AgentPlaysRole> createAgentPlaysRole(
      @RequestBody AgentPlaysRole agentPlaysRole) throws URISyntaxException {
   ...
   *     agentPlaysRole, or with status {@code 400 (Bad Request)} if the agentPlaysRole is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the agentPlaysRole couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/agent-plays-roles")
  public ResponseEntity<AgentPlaysRole> updateAgentPlaysRole(
      @RequestBody AgentPlaysRole agentPlaysRole) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, agentPlaysRole.getId().toString()))
        .body(result);
  }
   ...
   *     agentPlaysRole, or with status {@code 404 (Not Found)}.
   */
