...
public class ActivityInstantiatedResource {
   ...
   *     activityInstantiatedDTO, or with status {@code 400 (Bad Request)} if the
   *     activityInstantiated has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/activity-instantiateds")
  public ResponseEntity<ActivityInstantiatedDTO> createActivityInstantiated(
      @RequestBody ActivityInstantiatedDTO activityInstantiatedDTO) throws URISyntaxException {
    log.debug("REST request to save ActivityInstantiated : {}", activityInstantiatedDTO);
    if (activityInstantiatedDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new activityInstantiated cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     activityInstantiatedDTO, or with status {@code 400 (Bad Request)} if the
   *     activityInstantiatedDTO is not valid, or with status {@code 500 (Internal Server Error)} if
   *     the activityInstantiatedDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/activity-instantiateds")
  public ResponseEntity<ActivityInstantiatedDTO> updateActivityInstantiated(
      @RequestBody ActivityInstantiatedDTO activityInstantiatedDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, activityInstantiatedDTO.getId().toString()))
        .body(result);
  }
   ...
   *     activityInstantiatedDTO, or with status {@code 404 (Not Found)}.
   */
  ...
  public ResponseEntity<ActivityInstantiatedDTO> getActivityInstantiated(@PathVariable Long id) {
    ...
    Optional<ActivityInstantiatedDTO> activityInstantiatedDTO =
        activityInstantiatedService.findOne(id);
