...
public class EnactionDescriptionResource {
   ...
   *     enactionDescription, or with status {@code 400 (Bad Request)} if the enactionDescription
   *     has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/enaction-descriptions")
  public ResponseEntity<EnactionDescription> createEnactionDescription(
      @RequestBody EnactionDescription enactionDescription) throws URISyntaxException {
   ...
   *     enactionDescription, or with status {@code 400 (Bad Request)} if the enactionDescription is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the enactionDescription
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/enaction-descriptions")
  public ResponseEntity<EnactionDescription> updateEnactionDescription(
      @RequestBody EnactionDescription enactionDescription) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, enactionDescription.getId().toString()))
        .body(result);
  }
  ...
  public List<EnactionDescription> getAllEnactionDescriptions(
      @RequestParam(required = false) String filter) {
    if ("theplain-is-null".equals(filter)) {
      log.debug("REST request to get all EnactionDescriptions where thePlain is null");
      return StreamSupport.stream(enactionDescriptionRepository.findAll().spliterator(), false)
   ...
   *     enactionDescription, or with status {@code 404 (Not Found)}.
   */
