...
public class ProcessAgendaResource {
   ...
   *     processAgendaDTO, or with status {@code 400 (Bad Request)} if the processAgenda has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/process-agenda")
  public ResponseEntity<ProcessAgendaDTO> createProcessAgenda(
      @RequestBody ProcessAgendaDTO processAgendaDTO) throws URISyntaxException {
    log.debug("REST request to save ProcessAgenda : {}", processAgendaDTO);
    if (processAgendaDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new processAgenda cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     processAgendaDTO, or with status {@code 400 (Bad Request)} if the processAgendaDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the processAgendaDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/process-agenda")
  public ResponseEntity<ProcessAgendaDTO> updateProcessAgenda(
      @RequestBody ProcessAgendaDTO processAgendaDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, processAgendaDTO.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of processAgenda
   *     in body.
   */
   ...
   *     processAgendaDTO, or with status {@code 404 (Not Found)}.
   */
