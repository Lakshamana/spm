...
public class CalendarDayResource {
   ...
   *     calendarDayDTO, or with status {@code 400 (Bad Request)} if the calendarDay has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/calendar-days")
  public ResponseEntity<CalendarDayDTO> createCalendarDay(
      @RequestBody CalendarDayDTO calendarDayDTO) throws URISyntaxException {
    log.debug("REST request to save CalendarDay : {}", calendarDayDTO);
    if (calendarDayDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new calendarDay cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     calendarDayDTO, or with status {@code 400 (Bad Request)} if the calendarDayDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the calendarDayDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, calendarDayDTO.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of calendarDays in
   *     body.
   */
   ...
   *     calendarDayDTO, or with status {@code 404 (Not Found)}.
   */
