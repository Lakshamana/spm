...
public class NodeResource {
   ...
   *     nodeDTO, or with status {@code 400 (Bad Request)} if the node has already an ID.
   ...
   *     nodeDTO, or with status {@code 400 (Bad Request)} if the nodeDTO is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the nodeDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/nodes")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, nodeDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the nodeDTO, or
