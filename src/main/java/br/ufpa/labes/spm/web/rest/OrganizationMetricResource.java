...
public class OrganizationMetricResource {
   ...
   *     organizationMetricDTO, or with status {@code 400 (Bad Request)} if the organizationMetric
   *     has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/organization-metrics")
  public ResponseEntity<OrganizationMetricDTO> createOrganizationMetric(
      @RequestBody OrganizationMetricDTO organizationMetricDTO) throws URISyntaxException {
    log.debug("REST request to save OrganizationMetric : {}", organizationMetricDTO);
    if (organizationMetricDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new organizationMetric cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     organizationMetricDTO, or with status {@code 400 (Bad Request)} if the
   *     organizationMetricDTO is not valid, or with status {@code 500 (Internal Server Error)} if
   *     the organizationMetricDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/organization-metrics")
  public ResponseEntity<OrganizationMetricDTO> updateOrganizationMetric(
      @RequestBody OrganizationMetricDTO organizationMetricDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, organizationMetricDTO.getId().toString()))
        .body(result);
  }
   ...
   *     organizationMetrics in body.
   */
  @GetMapping("/organization-metrics")
  public List<OrganizationMetricDTO> getAllOrganizationMetrics() {
   ...
   *     organizationMetricDTO, or with status {@code 404 (Not Found)}.
   */
