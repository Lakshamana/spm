...
public class ProcessMetricResource {
   ...
   *     processMetricDTO, or with status {@code 400 (Bad Request)} if the processMetric has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/process-metrics")
  public ResponseEntity<ProcessMetricDTO> createProcessMetric(
      @RequestBody ProcessMetricDTO processMetricDTO) throws URISyntaxException {
    log.debug("REST request to save ProcessMetric : {}", processMetricDTO);
    if (processMetricDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new processMetric cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     processMetricDTO, or with status {@code 400 (Bad Request)} if the processMetricDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the processMetricDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/process-metrics")
  public ResponseEntity<ProcessMetricDTO> updateProcessMetric(
      @RequestBody ProcessMetricDTO processMetricDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, processMetricDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of processMetrics
   *     in body.
   */
  ...
  public List<ProcessMetricDTO> getAllProcessMetrics(
    ...
    if ("themetricsuper-is-null".equals(filter)) {
      log.debug("REST request to get all ProcessMetrics where theMetricSuper is null");
      return processMetricService.findAllWhereTheMetricSuperIsNull();
    }
   ...
   *     processMetricDTO, or with status {@code 404 (Not Found)}.
   */
