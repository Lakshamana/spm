...
public class ProjectServicesImpl implements ProjectServices {
      ...
      for (Iterator<RequiredPeople> iterator2 = people.iterator(); iterator2.hasNext(); ) {
        ...
        if (requiredPeople instanceof ReqAgent) {
          ...
          if (reqAgent.getTheAgent() != null) {
            ...
            processXML.append(
                "<ROLE>"
                    + (reqAgent.getTheRole() != null ? reqAgent.getTheRole().getIdent() : "")
                    + "</ROLE>\n");
          ...
          if (reqWorkGroup.getTheWorkGroup() != null) {
            ...
            String WorkGroupType =
                reqWorkGroup.getTheWorkGroupType() != null
    ...
    for (Iterator<Normal> iterator = theNormal.iterator(); iterator.hasNext(); ) {
      ...
      for (Iterator<RequiredResource> iterator2 = resources.iterator(); iterator2.hasNext(); ) {
        ...
        if (requiredResource.getTheResource() != null) {
          String resource =
              requiredResource.getTheResource() != null
          ...
          String resourceName =
              requiredResource.getTheResource() != null
                  ...
                  : "";
          String resourceType =
              requiredResource.getTheResourceType() != null
          ...
          processXML.append(
              "<NORMAL>" + requiredResource.getTheNormal().getIdent() + "</NORMAL>\n");
          processXML.append(
              getPositionTag(
                  requiredResource.getId(), requiredResource.getClass().getSimpleName()));
  ...
  /**
   * DAOException = Nao encontrou o pocesso nao banco de dados WebapseeException = Processo ja foi
   * iniciado
   *
  ...
  private ProjectStatistic convertProjectToProjectStatistic(Project project) {
    ...
    for (Task task : tasks) {
      ...
      //			float estimatedTask =
      // processEstimationDAO.getHoursEstimationForTask(task.getTheNormal().getIdent());
      //			Time estimatedTaskTime = new Time((int) estimatedTask, (int) ((estimatedTask * 60) %
      // 60));
    ...
    int DECIMAL_SCALE = 2;
    float finishedTasksPercentage =
        percentageOf(finishedTasks, numberOfTasks, DECIMAL_SCALE).floatValue();
    float delayedTasksPercentage =
        percentageOf(delayedTasks, numberOfTasks, DECIMAL_SCALE).floatValue();
  ...
  private void loadOrgAssociatives(Element associatives) {
    ...
    while (iteraprs.hasNext()) {
      ...
      if (!this.isAssociativeExists(AgentPlaysRole.class, agent, role)) {
        ...
        if (role != null) apr.setTheRole(role);
        if (agent != null) apr.setTheAgent(agent);
        apr.setSinceDate(
            (LocalDate) this.buildAttribute(Date.class, aprElm.getChildText("Since_date")));
  ...
  private void loadEnactionDescription(Element processComponents) {
    ...
    while (iter.hasNext()) {
      ...
      enactionDesc.setActualBegin(
          (LocalDate) this.buildAttribute(Date.class, enactionDescElm.getChildText("ActualBegin")));
      enactionDesc.setActualEnd(
          (LocalDate) this.buildAttribute(Date.class, enactionDescElm.getChildText("ActualEnd")));
  ...
  private void loadRequiredResource(Element processComponents, Element organizational) {
    ...
    while (iter.hasNext()) {
      ...
      reqResource.insertIntoTheResourceType(
          (ResourceType) this.organizational.get(resourceTypeKey));
  ...
  private void loadConnection(Element processComponents) {
    ...
    while (iter.hasNext()) {
      ...
      if (connection instanceof ArtifactCon) {
        ...
        artifactCon.setTheArtifactType(
            (ArtifactType) this.organizational.get(artTypeElm.getAttributeValue("REF")));
  ...
  private void loadTask(Element processComponents) {
    ...
    // 	Task task = new Task();
    // 	task.setBeginDate((Date) this.buildAttribute(Date.class,
    // taskElm.getChildText("BeginDate")));
  ...
  private Object setAttributes(Element e, Object obj) throws WebapseeException {
    ...
    if (ident == null) {
      if (obj instanceof MetricDefinition) ident = e.getChildText("Name");
    }
  ...
  private Object persistObject(Object obj, String oldIdent) {
    ...
    // since ConnectionDAO.daoSave() is needed
    //
    if (obj instanceof Connection) {
  ...
  private boolean isAssociativeExists(Class classe, Object from, Object to) {
    ...
    if (classe.equals(AgentAffinityAgent.class))
      hql +=
          "obj.fromAffinity.ident = '"
              + ((Agent) from).getIdent()
              + "' AND obj.toAffinity.ident = '"
              + ((Agent) to).getIdent()
              + "'";
      ...
      hql +=
          "obj.theArtifact.ident = '"
              + ((Artifact) from).getIdent()
              + "' AND obj.theTask.theNormal.ident = '"
              + ((Task) to).getTheNormal().getIdent()
              + "'";
    else if (classe.equals(BranchConCondToActivity.class))
      hql +=
          "obj.theBranchCond.ident = '"
              + ((BranchConCond) from).getIdent()
              + "' AND obj.theActivity.ident = '"
              + ((Activity) to).getIdent()
              + "'";
    else if (classe.equals(BranchConCondToMultipleCon.class))
      hql +=
          "obj.theBranchCond.ident = '"
              + ((BranchConCond) from).getIdent()
              + "' AND obj.theMultipleCon.ident = '"
              + ((MultipleCon) to).getIdent()
              + "'";
    else if (classe.equals(ReqAgentRequiresAbility.class))
      hql +=
          "obj.theReqAgent.theNormal.ident = '"
              + ((ReqAgent) from).getTheNormal().getIdent()
              + "' "
              + "AND obj.theReqAgent.theRole.ident = '"
              + ((ReqAgent) from).getTheRole().getIdent()
              + "' "
              + "AND obj.theReqAgent.theAgent.ident = '"
              + ((ReqAgent) from).getTheAgent().getIdent()
              + "' "
              + "AND obj.theAbility.ident = '"
              + ((Ability) to).getIdent()
              + "'";
