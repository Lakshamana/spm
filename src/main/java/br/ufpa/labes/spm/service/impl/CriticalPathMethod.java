...
public class CriticalPathMethod {
  ...
  private Collection<Activity> lastActs = new LinkedList<Activity>();
  ...
  public List<String> getCriticalPath(String procIdent) throws DAOException {
    ...
    for (AuxiliarActivity auxAct : allActs) {
      if (auxAct.getBeginIsCritical() || auxAct.getEndIsCritical()) {
        ...
        if (decompIdents != null) ret.addAll(decompIdents);
  ...
  public List<String> getCriticalPath(String procIdent, IProcessDAO processDAO)
      throws DAOException {
    List<String> ret = new LinkedList<String>();
  ...
  public Collection<AuxiliarConnection> getActsSucessorsOfAnAct(Activity beforeAct)
      throws DAOException {
    ...
    if (connections != null) {
      for (Object objCon : connections) {
        ...
        if (con instanceof Sequence) kindDep = ((Sequence) con).getTheDependency().getKindDep();
        ...
        if (acts != null) {
          for (Object objAct : acts) {
            ...
            if (act instanceof Normal) {
              ...
              Collection<Activity> firstActsFromDec =
                  this.getFirstActsFromPModel(dec.getTheReferedProcessModel());
      ...
      if (superDec != null) {
        ...
        if (beforeAct instanceof Normal) {
          beforeActPlannedEnd =
              Date.from(
                  ((Normal) beforeAct).getPlannedEnd().atStartOfDay().toInstant(ZoneOffset.UTC));
  ...
  private Collection getSuccessors(Connection conn) {
    ...
    if (conn instanceof Sequence) {
      ...
      if (seq.getToActivity() != null) succ.add(seq.getToActivity());
      ...
      if (obj != null) {
        ...
        if (conFrom == null) {
          if (act instanceof Normal) {
            ...
            Collection<Activity> decFirstActs =
                this.getFirstActsFromPModel(dec.getTheReferedProcessModel());
  ...
  public Date getLatestPlannedEndFromProcessModel(ProcessModel pModel) {
    ...
    for (Object obj : actsFromPModel) {
      ...
      if (act instanceof Normal) {
        ...
        planEnd = this.getLatestPlannedEndFromProcessModel(decAct.getTheReferedProcessModel());
      }
  ...
  private void discoverCriticalPath() {
    ...
    for (AuxiliarActivity auxAct : allActs) {
      if (!auxAct.getBeginIsCritical() || !auxAct.getEndIsCritical()) {
        ...
        for (Activity crit : this.lastActs) {
          List<Path> pathsBetweenActs = this.getPathsBetweenActs(auxAct.getActivity(), crit);
  ...
  private class AuxiliarActivity {
    ...
    public AuxiliarActivity(Activity act, Boolean beginIsCritical, Boolean endIsCritical) {
      this.activity = act;
