...
public interface ProcessRepository extends JpaRepository<Process, Long> {
    ...
    @Query(value = "select distinct process from Process process left join fetch process.theAgents",
    ...
    @Query("select distinct process from Process process left join fetch process.theAgents")
    List<Process> findAllWithEagerRelationships();

    @Query("select process from Process process left join fetch process.theAgents where process.id =:id")
